...
async function source(): Promise<string> {
  try { return await readFile(new URL("./builtin-skills/project-management/SKILL.md", import.meta.url), "utf8"); }
  catch (error) { if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error; }
  const value = await useStorage("assets:builtin-skills").getItem<string>("project-management/SKILL.md");
  if (typeof value !== "string" || !value.startsWith("---")) throw new Error("找不到Project管理Skill发布资源");
  return value;
}

/** Install as an ordinary Personal Skill. Only update a previously installed, unmodified copy. */
export async function ensureProjectManagementSkill(chatHome: string) {
  const home = await ensureChatHome(chatHome);
  const path = resolve(home.agentDir, "skills", "project-management", "SKILL.md");
  const receipt = resolve(home.runtimeDir, "builtin-skills", "project-management.sha256");
