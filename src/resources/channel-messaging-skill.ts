...
const SKILL_NAME = "channel-messaging";
...
async function source(): Promise<string> {
  ...
  if (typeof value !== "string" || !value.startsWith("---")) throw new Error("找不到通道消息Skill发布资源");
  return value;
}

/** Install as an ordinary Personal Skill. Only update a previously installed, unmodified copy. */
export async function ensureChannelMessagingSkill(chatHome: string) {
