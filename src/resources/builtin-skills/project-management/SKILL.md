...
- 工作目录按用户名称创建，中文无需转成英文或ID；同名目录占用时目录自动加序号，显示名不变。id是独立稳定身份，通常省略由Backend分配；不要用随机ID冒充项目名称。`projects/<id>`是私有运行数据，用户工作目录来自工具返回的导航。
...
- 当前没有项目删除Tool。用户要求删除时明确说明缺口；不能用Shell删除项目目录、Registry条目或创建恢复标记来冒充Project Tool操作。
