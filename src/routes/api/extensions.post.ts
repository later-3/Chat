...
export default defineEventHandler(async (event) => {
  ...
  const value = body as { projectId?: unknown; cwd?: unknown; action?: unknown; path?: unknown };
  try {
    ...
    await togglePiExtension(cwd, value.path, value.action === "enable", project?.projectId);
    return { success: true, ...await listPiExtensions(cwd, project?.projectId) };
  } catch (error) {
    throw createError({
