...
export default defineEventHandler(async (event) => {
  ...
  const value = body as { cwd?: unknown; filePath?: unknown; disableModelInvocation?: unknown };
  try {
    const cwd = await resolveResourceCwd(value.cwd ?? process.cwd());
    ...
    await setSkillModelInvocation(cwd, value.filePath, value.disableModelInvocation);
    return { success: true };
  } catch (error) {
    throw createError({
