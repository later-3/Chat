...
import { updateLongAgentAgentGroup } from "../../../../long-agents/agent-group-service.js";
import { projectLongAgentResourceError } from "../../../../long-agents/http-error.js";

export default defineEventHandler(async (event) => {
  setResponseHeader(event, "Cache-Control", "no-store");
  try {
    return await updateLongAgentAgentGroup(
      getRouterParam(event, "longAgentId"),
      await readBody<unknown>(event),
    );
  } catch (error) {
    throw createError(projectLongAgentResourceError(error, "更新Agent Group"));
  }
});
