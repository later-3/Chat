 ...
 */
/**
 * LA6 B: mirror a Chat-owned conversation channel binding to NanoClaw so the platform adapter can route
 * inbound group messages to the bound conversation and return the platform delivery receipt. NanoClaw
 * never decides authorization; it only stores the identity Chat supplies.
 */
export async function syncNanoClawConversationChannel(input: {
  readonly instance: LongAgentInstanceConfig;
  ...
  /** Platform adapter instance (e.g. `telegram-work`); distinct from the Host instance in `instance`. */
  ...
  const data = await requestGateway(input.instance, "v1/conversation-channels", {
    method: "POST",
    body: JSON.stringify({
      schemaVersion: 1,
      action: input.action,
      ...
      // The default adapter instance normalizes to the channel type on both sides, so it is omitted;
      // a named instance must travel intact for NanoClaw's exact address matching.
      ...(input.channelInstance === undefined || input.channelInstance === input.channelType
        ? {}
        : { instance: input.channelInstance }),
      ...(input.platformId === undefined ? {} : { platformId: input.platformId }),
      ...(input.messagingGroupId === undefined ? {} : { messagingGroupId: input.messagingGroupId }),
      ...(input.threadId === undefined ? {} : { threadId: input.threadId }),
      ...(input.botPlatformId === undefined ? {} : { botPlatformId: input.botPlatformId }),
    }),
  });
  if (!isRecord(data) || data.schemaVersion !== 1 || data.bindingId !== input.bindingId
    || (data.status !== "active" && data.status !== "unbound")
    || !Number.isSafeInteger(data.revision) || Number(data.revision) < input.revision) {
    throw new Error(`NanoClaw ${input.instance.id}没有确认会话渠道绑定`);
  }
}
...
/** Provisioning needs the actual failure, while presence only needs availability. */
export async function requireNanoClawGateway(instance: LongAgentInstanceConfig): Promise<void> {
  const data = await requestGateway(instance, "v1/health");
  if (!isRecord(data) || data.schemaVersion !== 1 || data.ok !== true || data.instanceId !== instance.id) {
    throw new NanoClawGatewayError("NanoClaw健康响应与实例配置不匹配", 409);
  }
}

export async function checkNanoClawGateway(instance: LongAgentInstanceConfig): Promise<boolean> {
  try {
    await requireNanoClawGateway(instance);
    return true;
  } catch {
    return false;
  }
...
/** Chat owns the definition; this endpoint only applies Nano's trigger projection. */
export async function requestTaskProjection(instance: LongAgentInstanceConfig, body: unknown): Promise<unknown> {
  return requestGateway(instance, "v1/task-projections", { method: "POST", body: JSON.stringify(body) });
}
