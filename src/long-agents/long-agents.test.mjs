...
import { ensureDailyProject } from "../projects/registry.ts";
import { openChatSession } from "../chat-session.ts";
import { readChatSession } from "../session-read-model.ts";
import { readLongAgentState, writeLongAgentRegistry } from "./storage.ts";
import {
...
import { acceptLongAgentEvents, listLongAgents, syncLongAgentEvents } from "./bridge.ts";
import { executeLongAgentTurn } from "./runtime.ts";
import { collectChatLongAgentTurnMarkers } from "./session-turn.ts";
import readLongAgentConfigurationHandler from "../routes/api/long-agents/[longAgentId]/config.get.ts";
import updateLongAgentConfigurationHandler from "../routes/api/long-agents/[longAgentId]/config.put.ts";
...
test("legacy per-session bindings migrate to one Project Long Agent primary session", { concurrency: false }, async (t) => {
  ...
  assert.deepEqual(state.projectAgents, [{
    id: "project-long-agent:daily:nexus",
    projectId: "daily",
    longAgentId: "nexus",
...
test("LongAgent registry rejects unsafe Gateway URLs and duplicate Agent Group mappings", async (t) => {
  ...
  await assert.rejects(writeLongAgentRegistry(base, root), /必须使用HTTPS/);
  const instance = { id: "local", name: "Local", gatewayBaseUrl: "http://127.0.0.1:3000/webhook/chat-backend" };
  await assert.rejects(writeLongAgentRegistry({
    schemaVersion: 1,
  ...
  const agent = {
    ...
    nanoclawAgentGroupId: "ag-shared", defaultProjectId: "daily",
    inbox: {
...
test("LongAgent configuration is versioned, atomic, and keeps private routing out of its browser document", async (t) => {
  ...
  const enabledListJson = JSON.stringify(await listLongAgents({ projectId: "daily", chatHome }));
...
test("Chat Web Long Agent runs Pi natively and replays one stable Turn only once", { concurrency: false }, async (t) => {
  ...
  const opened = await openChatSession({ projectId: "daily", chatHome, sessionId: first.sessionId });
  assert.deepEqual(
  ...
  const projected = await readChatSession(first.sessionId, undefined, {}, "daily", chatHome);
  assert.deepEqual(
