...
export const BUILT_IN_PERSONAL_PROMPT_RESOURCES = [
  ...
  {
    schemaVersion: 1,
    id: "native-summary-web-projection",
    revisions: [{
      schemaVersion: 1, id: "native-summary-web-projection", revision: 1, kind: "experience",
      title: "原生压缩摘要与公共聊天恢复",
      purpose: "避免实时流正常但压缩后的快照无法被浏览器恢复。",
      content: "Pi SessionManager 是历史事实源，但原生 compactionSummary/branchSummary 不是 Web AgentMessage。统一在 Backend 读投影转换为已有 custom 展示合同，保留摘要、Entry ID、时间和原生存储；不能只在 Friend 放宽校验或让浏览器猜角色。验证必须覆盖压缩后读取、快照解析、刷新及再次发送；工具或最终文字正常不能代替恢复链验收。失联重连只 GET，不重新 POST。门禁为 session-read-model 的原生压缩投影测试与 friend-execution 的压缩后快照/非法消息合同测试，再用真实浏览器触发原生 Pi 压缩并继续交互。",
      tags: ["development", "session", "compaction", "frontend", "recovery"], status: "active",
      sources: [{ type: "manual", entryIds: [], context: "docs/development/experiences/native-summary-web-projection.md", capturedAt: "2026-09-19T15:40:00.000Z" }],
      author: { type: "user" }, createdAt: "2026-09-19T15:40:00.000Z",
    }],
  },
  {
    schemaVersion: 1,
