...
import { ensureDailyProject, openProject, readProjectRegistry, resolveProjectContext } from "./registry.ts";
import { resolveChatSystemTools, listChatSystemTools } from "../tools/registry.ts";
import { writeProjectChatConfig, resolveChatConfig } from "../chat-config.ts";
import { updateAgentDurableConfig } from "../workflows/agent-model-config.ts";
import { fileRevision } from "../persistence/versioned-file.ts";
import { ensureProjectManagementSkill } from "../resources/project-management-skill.ts";
import { createChatPiAgentSession } from "../agents/pi-agent-session.ts";
...
test("unset removes empty ancestors so Personal defaults really become effective", async (t) => {
  const { ok, context } = await fixture(t);
  const { writeChatRootConfig } = await import("../chat-config.ts");
...
test("existing full-document writers can repair malformed files while patches refuse corrupt state", async (t) => {
  ...
  assert.equal((await resolveChatConfig("daily", context.chatHome)).project.defaultWorkflowId, "memory");
  const { agentModelConfigPath, writeAgentDurableConfig, readAgentDurableConfig } = await import("../workflows/agent-model-config.ts");
