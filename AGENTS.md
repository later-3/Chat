...
5. Chat通道通过明确合同接入上位OPC-OS Chat能力，不读取或写入其他通道的私有状态。
6. Runtime、模型、工具、记忆和存储必须可替换，不能散落在UI或路由代码中。
7. 外部调用必须有超时、结构校验、错误脱敏、幂等或明确的重复执行语义。
8. 配置、密钥和路径从环境注入；真实密钥不得进入源码、文档、Trace或Git。
