...
function ExecutionRow({ value }: { value: ToolExecutionSummary }) {
  return (
    <article className="tool-execution-row">
      <span className={`tool-execution-status tool-execution-status--${value.status}`}>
        <Activity size={14} />
      </span>
      <div>
        <strong>{EXECUTION_STATUS_LABELS[value.status] ?? value.status}</strong>
        <small>
          {new Date(value.started_at).toLocaleString()} · config r{value.config_revision}
        </small>
      </div>
      <dl>
        <div>
          <dt>模型调用</dt>
          <dd>{value.model_call_count}</dd>
        </div>
        <div>
          <dt>内部 Tool 调用</dt>
          <dd>{value.internal_tool_call_count}</dd>
        </div>
        <div>
          <dt>Token 总量</dt>
          <dd>{value.tokens.input + value.tokens.output}</dd>
        </div>
        <div>
          <dt>耗时</dt>
          <dd>{duration(value.duration_ms)}</dd>
        </div>
      </dl>
    </article>
  );
  ...
  if (loading)
    return (
      <main className="tool-page">
        <div className="workflow-empty">正在加载Tool目录…</div>
      </main>
    );
  if (!config)
    return (
      <main className="tool-page">
        <div className="workflow-empty">当前没有已注册的Tool。</div>
      </main>
    );
    ...
    <main className="tool-page">
      <header className="tool-page-header">
        <div className="tool-page-title">
          ...
          <div>
            <p className="eyebrow">SERVER TOOL CATALOG · r{config.revision}</p>
            <h1>{config.name}</h1>
        ...
        <div className="tool-page-header-actions">
          <span className={config.runtime.available ? "tool-runtime-ok" : "tool-runtime-off"}>
            <ShieldCheck size={14} />
            {config.runtime.available ? "RPC运行时可用" : "RPC运行时不可用"}
          </span>
          <button disabled={saving} onClick={() => void load()} type="button">
            <RefreshCw size={15} />
            刷新
          </button>
        </div>
      ...
      <section className="tool-page-grid">
        <div className="tool-config-card">
          <div className="tool-card-heading">
            ...
            <small>修改后对新Workflow快照生效</small>
          </div>
          <label className="tool-enabled-row">
            <span>
              <strong>启用pi Agent Tool</strong>
              <small>禁用后不能发起新pi运行</small>
            </span>
            <input
          ...
          <div className="settings-grid">
            ...
            <label className="settings-field">
              <span>模型</span>
              <select
                disabled={blocked || saving || !provider}
            ...
            <label className="settings-field">
              ...
              <select
                disabled={blocked || saving}
                onChange={(event) => setConfig({ ...config, thinking_level: event.target.value })}
                value={config.thinking_level}
              >
                {config.thinking_levels.map((value) => (
            ...
            <label className="settings-field">
              ...
              <input
                ...
                min={1}
                onChange={(event) =>
                  setConfig({ ...config, max_model_calls: Number(event.target.value) })
                }
                type="number"
                value={config.max_model_calls}
              />
            </label>
            <label className="settings-field">
              <span>总超时（秒）</span>
          ...
          <label className="settings-field tool-full-field">
            <span>工作目录</span>
            <input
              disabled={blocked || saving}
              onChange={(event) => setConfig({ ...config, working_directory: event.target.value })}
              value={config.working_directory}
            />
            <small>
              必须位于后端允许的根目录内：{config.runtime.allowed_working_roots.join("、")}
            </small>
          </label>
          <label className="settings-field tool-full-field">
            <span>System Prompt（系统指令）</span>
            <textarea
              disabled={blocked || saving}
          ...
          <div className="tool-selection">
            <div>
              <strong>pi内部Tool</strong>
              <span>这里只能选择真实存在的pi Tool；运行时每次仍单独审批参数</span>
            </div>
            <div>
                    ...
                    onChange={(event) =>
                      setConfig({
                        ...
                        allowed_tools: event.target.checked
                          ...
                          : config.allowed_tools.filter((value) => value !== tool),
                      })
                    }
                    type="checkbox"
                  />
          ...
          <div className="tool-save-row">
            <span>{saved ? "已保存新Revision" : "配置不等于执行授权"}</span>
            <button
              disabled={blocked || saving || config.allowed_tools.length === 0}
              onClick={() => void save()}
              type="button"
            >
              <Save size={15} />
        ...
        <aside className="tool-observability-card">
          <div className="tool-card-heading">
            ...
            <small>{executions.length} 条最近记录</small>
          </div>
          <div className="tool-runtime-facts">
            <div>
              ...
              <span>
                <small>集成方式</small>
                <strong>JSONL RPC 子进程</strong>
              </span>
            </div>
            <div>
              <ShieldCheck size={16} />
              <span>
                <small>Provider Gate</small>
                <strong>每次模型调用</strong>
              </span>
            </div>
            <div>
              <Wrench size={16} />
              <span>
                <small>Tool Gate</small>
                <strong>每次内部Tool</strong>
              </span>
            </div>
            <div>
              <Clock3 size={16} />
              <span>
                <small>超时上限</small>
                <strong>{config.timeout_seconds}秒</strong>
              </span>
            </div>
          </div>
          <div className="tool-execution-list">
            {executions.length ? (
              executions.map((value) => <ExecutionRow key={value.id} value={value} />)
            ) : (
              <div className="tool-no-executions">
                还没有 pi 运行记录。在配置中心的 Workflow 开发验证中运行 pi Agent 后，模型调用、内部
                Tool 调用、Token 和耗时会出现在这里。
              </div>
            )}
          </div>
        </aside>
