...
function ExecutionRow({ value }: { value: ToolExecutionSummary }) {
  return (
    <article className="tool-execution-row">
      ...
      <dl>
        <div><dt>模型</dt><dd>{value.model_call_count}</dd></div>
        <div><dt>Tool</dt><dd>{value.internal_tool_call_count}</dd></div>
        <div><dt>Tokens</dt><dd>{value.tokens.input + value.tokens.output}</dd></div>
      ...
      <section className="tool-page-grid">
        <div className="tool-config-card">
          ...
          <div className="settings-grid">
            ...
            <label className="settings-field"><span>Thinking</span><select disabled={blocked || saving} onChange={(event) => setConfig({ ...config, thinking_level: event.target.value })} value={config.thinking_level}>{config.thinking_levels.map((value) => <option key={value} value={value}>{value}</option>)}</select></label>
            <label className="settings-field"><span>最大模型调用</span><input disabled={blocked || saving} max={100} min={1} onChange={(event) => setConfig({ ...config, max_model_calls: Number(event.target.value) })} type="number" value={config.max_model_calls} /></label>
          ...
          <label className="settings-field tool-full-field"><span>System Prompt</span><textarea disabled={blocked || saving} onChange={(event) => setConfig({ ...config, system_prompt: event.target.value })} rows={5} value={config.system_prompt} /></label>
        ...
        <aside className="tool-observability-card">
          ...
          <div className="tool-execution-list">
            {executions.length ? executions.map((value) => <ExecutionRow key={value.id} value={value} />) : <div className="tool-no-executions">还没有pi执行记录。在“工作流”中运行pi Agent后，模型调用、Tool次数、Token和耗时会出现在这里。</div>}
          </div>
        </aside>
      </section>
    </main>
