...
async function checkedJson(response: Response): Promise<ExecutionDraftView> {
  if (!response.ok) {
    const payload = await response.json() as { detail?: string };
    throw new Error(payload.detail ?? `ExecutionDraft请求失败：HTTP ${response.status}`);
  }
  return response.json() as Promise<ExecutionDraftView>;
}

export async function getExecutionDraft(draftId: string): Promise<ExecutionDraftView> {
  return checkedJson(await fetch(`${API_BASE_URL}/api/execution-drafts/${draftId}`));
}
...
export async function reviseExecutionDraft(
  ...
  return checkedJson(await fetch(`${API_BASE_URL}/api/execution-drafts/${draft.id}`, {
    method: "PUT",
