...
import { type AgentProfile, agentModelOptions, listAgents, updateAgent } from "./agent-api";
    ...
    <main className="agent-layout">
      <header className="agent-page-header">
        <div>
          ...
          <h1>Agent 配置</h1>
          <p>
            这里配置版本化 Agent 档案；每次 Workflow
            运行会取得一个版本快照，实际模型请求仍逐次审批。
          </p>
        </div>
        ...
        <aside className="agent-list" aria-label="Agent列表">
          {loading && (
            <div className="agent-loading">
              <LoaderCircle className="workflow-spin" size={17} />
              读取中
            </div>
          )}
          {!loading && agents.length === 0 && (
            <div className="agent-loading">当前没有可配置的Agent</div>
          )}
          {agents.map((profile) => (
            <button
              className={
          ...
          <div className="agent-handoff-note">
            <GitMerge size={16} />
            <p>
              <strong>显式 Agent 交接</strong>
              <span>规划/审校双Agent；成语接龙Agent甲/乙均由确定性节点交接。</span>
            </p>
          </div>
        </aside>
            ...
            <>
              ...
              <div className="agent-form-grid">
                ...
                <label>
                  <span>状态</span>
                  <select
                    disabled={saving || blocked || requiredAgentIds.has(draft.id)}
                    onChange={(event) =>
                      setDraft({ ...draft, enabled: event.target.value === "enabled" })
                    }
                ...
                <label>
                  ...
                  <select
                    disabled={saving || blocked}
                    onChange={(event) => {
                      const provider = providers.find((value) => value.id === event.target.value);
                      setDraft({
                        ...draft,
                ...
                <label className="agent-form-wide">
                  ...
                  <small>
                    定义这个Agent如何理解任务与产出；本次调用还可以在发送前审批中临时修改。
                  </small>
                  <textarea
                    disabled={saving || blocked}
              ...
              {saved && (
                <p className="agent-form-success">
                  <Check size={14} />
                  已保存，新 Workflow 将使用 Agent 版本 {draft.revision}
                </p>
              )}
              <div className="agent-form-actions">
                <button
                  disabled={
                    !dirty || saving || blocked || !draft.name.trim() || !draft.instructions.trim()
                  }
                  onClick={() => void save()}
                  type="button"
                >
                  <Save size={15} />
                  {saving ? "保存中…" : "保存Agent配置"}
                </button>
              </div>
            </>
          ) : !loading ? (
            <div className="agent-loading">请选择Agent</div>
          ) : null}
        </div>
      </section>
    </main>
