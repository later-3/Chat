...
| Product Store | `chat-product-store.v21`版本化JSON Adapter；v20新增Provider中立的`MemorySessionImport`批次，v21原子迁移并新增`MemoryAgentOperation`、`MemoryAgentWriteCandidate`、`MemoryAgentWriteDecision`及固定`direct@3/4/5`完整/只读/只整理系统Definition/Revision/View，历史Run与v20及更早事实原样保留。Session Import批次冻结来源快照/预览Hash并引用统一`MemoryWriteIntent/Result`，不把Codex Session伪装成Product Session；Memory Agent Operation以输入/来源Hash、Provider请求次数和`dispatching/succeeded/failed/outcome_unknown`耐久记录外部/模型边界，候选批准后才创建真正的Write Intent。Store继续拥有不可变`AgentVersion`、Provider中立`WorkflowMemoryQuery/Snapshot/Context`及Memory Write事实；Memory Direct Attempt把Context ID/Revision/Hash纳入Direct Input Manifest。用户会话Prompt与兼容Agent Prompt Revision正文仍位于可见Markdown文件；Direct Run以Prompt Assembly v2冻结System、正式Messages、显式能力或`inherit_runtime_default`解析模式和Options，完成Extension绑定后的真实Provider Payload由Prompt Review冻结，实际Runtime Manifest Hash进入Pi Journal；多节点Run继续用v3冻结各节点有效Prompt与同一份会话上下文 |
| Workflow | Vercel Workflow解释不可变RunSpec，承担耐久步骤、暂停、恢复与Checkpoint；发送级节点配置由Node Catalog字段、Blueprint白名单和具体Definition默认值共同描述并由Compiler冻结。既有`direct@1 / direct-agent.v1`仍是单节点“执行 Agent（逐次提示词审核）”；`direct@2 / memory-direct.v1`固定执行`memory.query → agent.direct → memory.write`；`memory-agent-direct.v1`只接受3种冻结组合：`direct@3`完整`retrieve → direct → write`、`direct@4`只查询`retrieve → direct`、`direct@5`只整理`direct → write`。检索Agent只对同次Provider原始结果作引用筛选并冻结`WorkflowMemoryContext`；写入Agent只生成绑定Direct Candidate/证据Hash的待审核候选，用户以Candidate revision/hash批准后才创建统一Memory Write Intent/Outbox，拒绝不产生外部写。两类Agent模型/Provider边界均先持久化`MemoryAgentOperation`，发出请求后失联收敛为`outcome_unknown`且不猜测性重跑。Pi Direct Agent节点可精确绑定`AgentVersion ID + Hash`；系统Definition保存时派生个人已发布版本，个人Definition原子发布下一Revision；当前DSH Session临时差异只进入结构化`agent_configuration`并在每个Run冻结，不写回Agent或Workflow |
...
| Memory | 运行基础、Memory Direct、Session导入、双Provider查询比较、Retrieval/Write Agent、人工Candidate Decision与DSH Memory管理页已交付。普通`pnpm dev`默认`CHAT_MEMORY_MODE=off`，不准备工件、不检查端口、不启动Sidecar；显式`memorycore / memmy / compare`才启动固定Provider并让API/Workflow冻结同一Registry。用户可在Composer选择`direct@4`“只查询 Memory 后回答”、`direct@5`“只整理为 Memory 候选”或`direct@3`“Memory Agent 增强执行”；只查询没有写入节点，只整理没有Workflow Memory Context，完整组合按`retrieve → direct → write candidate`运行。真正外部Write始终只在用户批准候选后通过统一Write/对账状态机执行。Chat/Codex Session Preview/Import、条目去重和双Provider比较继续按需调用，不增加常驻扫描或第二套事实。已通过memmy与MemoryCore真实HTTP、真实Codex Session、百炼`qwen3.7-plus` Memory Agent、默认-off DSH Memory管理Chromium、memmy断响应与Sidecar孤儿恢复门；显式memmy + 真实百炼 + DSH Chromium串行纵向也已通过，依次验证`direct@4/5/3`的提示词Context边界、候选批准、Provider物化和刷新恢复 |
...
7. Memory当前授权纵向已完成：运行基础、Memory Direct、Chat/Codex Session增量导入、双Provider比较、`direct@3/4/5`完整/只查询/只整理组合、人工Candidate Decision和DSH管理表面均已完成；分层真实门与显式Provider的DSH串行浏览器纵向均已通过。普通开发仍默认`off`。Browser Provider顺延为下一候选纵向，但没有新的用户授权前不得开工。
...
6. 当前默认“规划执行工作流”和既有`direct@1`仍不声明Memory节点；普通开发默认`CHAT_MEMORY_MODE=off`，不会准备/启动Sidecar。只有显式Memory运行模式才准备Sidecar，且只有用户选择`direct@2/3/4/5`、批准Memory Agent候选、确认Session导入或主动发起比较Preview才调用Memory。DSH管理页/窄代理不绕过该模式，也不把Provider配置、外部对象ID或正文缓存为浏览器/Bridge事实。
