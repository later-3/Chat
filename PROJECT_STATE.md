...
| Product Store | `chat-product-store.v21`版本化JSON Adapter；v20新增Provider中立的`MemorySessionImport`批次，v21原子迁移并新增`MemoryAgentOperation`、`MemoryAgentWriteCandidate`、`MemoryAgentWriteDecision`及固定`direct@3`系统Definition/Revision/View，历史Run与v20及更早事实原样保留。Session Import批次冻结来源快照/预览Hash并引用统一`MemoryWriteIntent/Result`，不把Codex Session伪装成Product Session；Memory Agent Operation以输入/来源Hash、Provider请求次数和`dispatching/succeeded/failed/outcome_unknown`耐久记录外部/模型边界，候选批准后才创建真正的Write Intent。Store继续拥有不可变`AgentVersion`、Provider中立`WorkflowMemoryQuery/Snapshot/Context`及Memory Write事实；Memory Direct Attempt把Context ID/Revision/Hash纳入Direct Input Manifest。用户会话Prompt与兼容Agent Prompt Revision正文仍位于可见Markdown文件；Direct Run以Prompt Assembly v2冻结System、正式Messages、显式能力或`inherit_runtime_default`解析模式和Options，完成Extension绑定后的真实Provider Payload由Prompt Review冻结，实际Runtime Manifest Hash进入Pi Journal；多节点Run继续用v3冻结各节点有效Prompt与同一份会话上下文 |
| Workflow | Vercel Workflow解释不可变RunSpec，承担耐久步骤、暂停、恢复与Checkpoint；发送级节点配置由Node Catalog字段、Blueprint白名单和具体Definition默认值共同描述并由Compiler冻结。既有`direct@1 / direct-agent.v1`仍是单节点“执行 Agent（逐次提示词审核）”；`direct@2 / memory-direct.v1`固定执行`memory.query → agent.direct → memory.write`，不修改也不隐式包裹旧Direct；新增独立`direct@3 / memory-agent-direct.v1`固定执行`agent.memory_retrieve → agent.direct → agent.memory_write`。检索Agent只对同次Provider原始结果作引用筛选并冻结`WorkflowMemoryContext`；写入Agent只生成绑定Direct Candidate/证据Hash的待审核候选，用户以Candidate revision/hash批准后才创建统一Memory Write Intent/Outbox，拒绝不产生外部写。两类Agent模型/Provider边界均先持久化`MemoryAgentOperation`，发出请求后失联收敛为`outcome_unknown`且不猜测性重跑。Pi Direct Agent节点可精确绑定`AgentVersion ID + Hash`；系统Definition保存时派生个人已发布版本，个人Definition原子发布下一Revision；当前DSH Session临时差异只进入结构化`agent_configuration`并在每个Run冻结，不写回Agent或Workflow |
...
| Memory | 运行基础、独立Memory Direct、Session导入、双Provider查询比较、Retrieval/Write Agent、人工Candidate Decision与DSH Memory管理页已交付。统一setup/dev接受显式`off / memorycore / memmy / compare`：普通`pnpm dev`默认`off`，不准备工件、不检查Memory端口、不启动Sidecar，API/Workflow Registry严格为空；DSH Memory管理页与其同源Host代理不改变此默认，也无需Sidecar即可启动并在`off`模式诚实显示空Provider/资源。启用模式才准备固定源码并按production `18960/18970`、debug `19960/19970`及各自数据根启动选中的本地HTTP Sidecar。memmy与Tencent MemoryCore都实现同一Workflow Query/Write/Reconcile Port，API与Workflow在打开Store/恢复Run前冻结同一`CHAT_MEMORY_MODE`。用户显式选择`direct@2`“Memory 增强执行 Agent”后，Query结果先冻结为Context，再作为当前请求前的不可信历史交给同一Pi Direct Agent；候选成功后才按节点配置写回来源Message。用户显式选择`direct@3`“Memory Agent 增强执行”后，Retrieval Agent只从Provider返回的原始结果中选择引用，Write Agent只整理绑定可见Message/Direct Candidate证据的候选；真正外部Write只在用户批准候选后通过统一Write/对账状态机执行。Chat Product Session与本机Codex Session可按需列表、只读预览并经`conversation-turns.v1`只采用user/assistant可见正文；确认来源/预览Hash后创建增量批次，同一条目重跑零新增，外部写入继续复用统一Write/对账状态机。比较Preview把同一来源namespace、查询和预算并行交给至少两个Provider，展示各自命中/采用正文、精确正文交集、唯一项与共享标签；Provider score明确禁止跨实现比较，Preview不写Product Store或Memory。Codex文件只在调用相关Query/Preview/Import时扫描，不增加常驻服务。memmy写入为`synchronous`且当前只允许绑定单Principal的Chat专属数据库；本地无模型MemoryCore为`accepted_only`。已实际通过：memmy与MemoryCore真实HTTP门；真实Codex Session门（50个来源、抽样8条消息，转换为4个条目）；百炼`qwen3.7-plus` Memory Agent真实门3/3（Retrieval选择Provider原始index 1，Write产生1个候选）；默认-off的DSH Memory管理真实Chromium门2/2（空Provider、三页签、桌面/390px无横向溢出）；以及真实memmy断响应门（`outcome_unknown`后同身份只读对账，最终仅1个外部对象）和Sidecar wrapper `SIGKILL`孤儿恢复门（子进程组回收、端口释放）。仍未运行“显式启用Provider的完整`direct@3` DSH纵向”，不能用上述独立Provider或默认-off浏览器门替代 |
...
7. Memory是当前已授权纵向。运行基础、独立Memory Direct Workflow、受控Chat/Codex Session增量导入、双Provider查询比较、`direct@3` Retrieval/Write Agent、人工Candidate Decision和DSH管理表面均已完成；memmy/MemoryCore真实HTTP、Codex Session（50来源、抽样8消息→4条目）、百炼`qwen3.7-plus` Memory Agent（3/3，原始index 1→1候选）及默认-off DSH Memory管理真实Chromium（2/2）均已通过。显式启用Provider的完整`direct@3` DSH纵向仍未运行，不能由这些分层门替代。不得让旧Memory Planning形态限制当前目标。Browser Provider顺延为下一候选纵向。
...
6. 当前默认“规划执行工作流”和既有`direct@1`仍不声明Memory节点；普通开发默认`CHAT_MEMORY_MODE=off`，不会准备/启动Sidecar。只有显式Memory运行模式才准备Sidecar，且只有用户选择独立Memory Workflow、批准Memory Agent候选、确认Session导入或主动发起比较Preview才调用Memory。DSH管理页/窄代理不绕过该模式，也不把Provider配置、外部对象ID或正文缓存为浏览器/Bridge事实；默认-off DSH管理页真实Chromium门已通过，但显式启用Provider的完整`direct@3` DSH纵向仍未执行。
