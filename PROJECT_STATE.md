...
| Product Store | 当前写代为`chat-product-store.v26`。v24监督执行事实与v25治理事实原样保留；v25→v26只增加Memory Session Import、Memory Agent Operation/Candidate/Decision空集合及固定Memory Direct系统Definition。旧Revision/View保持可精确回放，旧代只读；Application重验确定性Validation、Evidence、Prompt与独立治理Attempt。监督基础见[监督执行v24基础](./docs/architecture/supervised-execution-v24-foundation-as-built.md)，治理事实见[Capability治理](./docs/architecture/capability-governance-as-built.md) |
...
| 全项目生命周期管理 | Chat拥有Project/Stage/Milestone/Update监督账本、Content Production Work/Claim/Block/Review/Handoff/Outcome/Practice、四类通用Profile、具体Configuration、六类Agent Context、Maintenance Plan、Content Lab有界Resource Context及Project Home/Timeline/Review/Query。当前Application与新API Opening写`project-agent-coordination.v3`并绑定精确目标的`project-agent-context.v2`；Opening v1/v2与Context v1严格只读保留，且普通Opening不读取外部事项Provider。DSH提供空白会话可达的全局项目入口和四视图。Plane默认不装配，也不出现在当前Catalog的Agent/Fragment/Tool/详情、Workflow/Opening/DSH/常规文档；历史Prompt Revision与Binding/Operation/Outbox保留且不会阻塞普通队列 |
...
6. Project管理内核与DSH四视图已经收敛；Content Lab监督执行已完成Contracts/Domain/Product Store v24基础，Application、Workflow、Pi、API、Bridge与Trace纵向仍须按后续阶段显式接入。Maintenance自动调度及Document/Code/Media具体Viewer仍需独立授权。
7. Browser Provider仍是下一候选纵向；Memory产品与Workflow基线已经恢复，但运行默认关闭。真实Provider部署、长期记忆策略与默认启用仍须新的用户场景和明确授权。
8. 四个标杆项目提炼出的三个治理组件已经在DSH按规则、要求、经验分类展示并可按会话勾选；同一选择进入Planner、Executor和独立Governance Reviewer。确定性纵向覆盖治理通过提交、blocking阻断、Runtime重启恢复；真实DSH浏览器覆盖展示、勾选、Git来源和治理节点完整Prompt预览。
...
9. 没有为本次Agent管理与Project管理内核改动运行真实付费模型门；普通Direct默认继承Pi CLI真实编码能力；历史`project_bootstrap`不会出现在默认Prompt/Agent/Workflow面，只有显式专项Provider启用后才可恢复兼容路径。DSH Tool/Skill/Plugin尚未接成Pi可执行能力，页面不得把目录可见性冒充执行能力。
...
- Project管理纵向由Contracts、Application、Store、Bridge合同测试与DSH真实Browser总门共同验证；历史
  Bootstrap浏览器场景不属于默认产品入口，也不启动外部项目Provider或付费模型。
