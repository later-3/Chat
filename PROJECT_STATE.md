...
| 全项目生命周期管理 | Chat拥有Project/Stage/Milestone/Update监督账本、Content Production Work/Claim/Block/Review/Handoff/Outcome/Practice、四类通用Profile、具体Configuration、六类Agent Context、Maintenance Plan、Content Lab有界Resource Context及Project Home/Timeline/Review/Query。当前Application与新API Opening写`project-agent-coordination.v3`并绑定精确目标的`project-agent-context.v2`；Opening v1/v2与Context v1严格只读保留。DSH提供空白会话可达的全局项目入口和四视图。 |
...
9. 没有为本次Agent管理与Project管理内核改动运行真实付费模型门；普通Direct默认继承Pi CLI真实编码能力。DSH Tool/Skill/Plugin尚未接成Pi可执行能力，页面不得把目录可见性冒充执行能力。
...
- Project管理纵向由Contracts、Application、Store、Bridge合同测试与DSH真实Browser总门共同验证。
