...
| Memory | 运行基础已重新启用并默认安全关闭。统一setup/dev接受显式`off / memorycore / memmy / compare`：`off`不准备工件、不检查Memory端口、不启动Sidecar且API/Workflow Registry严格为空；启用模式才准备固定源码并按production `18960/18970`、debug `19960/19970`及各自数据根启动选中的本地HTTP Sidecar。memmy与Tencent MemoryCore都实现同一Workflow Query/Write/Reconcile Port，API与Workflow在打开Store/恢复Run前冻结同一`CHAT_MEMORY_MODE`，遗留凭据不能绕过`off`。memmy写入为`synchronous`且当前只允许绑定单Principal的Chat专属数据库；本地无模型MemoryCore为`accepted_only`，只读发现真实同session L1后才可升级`materialized`。现有Memory Planning仍是旧纵向证据；Memory增强Direct、Session导入、Memory Agent与管理表面尚未交付 |
...
7. Memory是当前已授权纵向。运行基础完成后依次交付独立Memory Direct Workflow、受控Chat/Codex Session导入、双Provider对比、Retrieval/Write Agent与DSH管理表面；不得让旧Memory Planning形态限制当前目标。Browser Provider顺延为下一候选纵向。
...
6. 当前默认“规划执行工作流”仍不声明Memory节点；只有显式Memory运行模式才准备/启动Sidecar。新的Memory Direct Workflow、整Session导入、Memory Agent和管理表面尚未实现，不能因运行基础可用就宣称完整产品能力已完成。
