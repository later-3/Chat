...
| 全项目生命周期管理 | Chat拥有Project/Stage/Milestone/Update监督账本、Content Production Work/Claim/Block/Review/Handoff/Outcome/Practice、四类通用Profile、具体Configuration、六类Agent Context、Maintenance Plan、Content Lab有界Resource Context及Project Home/Timeline/Review/Query。普通Opening写`project-agent-coordination.v2`且不读取外部事项Provider；v1严格保留历史读取。DSH提供空白会话可达的全局项目入口和四视图。Plane默认不装配、不出现在Catalog/Workflow/Opening/DSH/常规文档；历史Binding/Operation/Outbox保留且不会阻塞普通队列 |
