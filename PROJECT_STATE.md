...
| Memory | 运行基础、独立Memory Direct与Session导入已交付。统一setup/dev接受显式`off / memorycore / memmy / compare`：`off`不准备工件、不检查Memory端口、不启动Sidecar且API/Workflow Registry严格为空；启用模式才准备固定源码并按production `18960/18970`、debug `19960/19970`及各自数据根启动选中的本地HTTP Sidecar。memmy与Tencent MemoryCore都实现同一Workflow Query/Write/Reconcile Port，API与Workflow在打开Store/恢复Run前冻结同一`CHAT_MEMORY_MODE`。用户显式选择“Memory 增强执行 Agent”后，Query结果先冻结为Context，再作为当前请求前的不可信历史交给同一Pi Direct Agent；候选成功后才按节点配置写回来源Message。Chat Product Session与本机Codex Session可按需列表、只读预览并经`conversation-turns.v1`只采用user/assistant可见正文；确认来源/预览Hash后创建增量批次，同一条目重跑零新增，外部写入继续复用统一Write/对账状态机。Codex文件只在调用来源Query/Preview/Import时扫描，不增加常驻服务。memmy写入为`synchronous`且当前只允许绑定单Principal的Chat专属数据库；本地无模型MemoryCore为`accepted_only`。双Provider评测、Memory Agent与DSH管理表面尚未交付 |
...
7. Memory是当前已授权纵向。运行基础、独立Memory Direct Workflow和受控Chat/Codex Session增量导入已完成；下一步依次交付双Provider对比、Retrieval/Write Agent与DSH管理表面。不得让旧Memory Planning形态限制当前目标。Browser Provider顺延为下一候选纵向。
...
6. 当前默认“规划执行工作流”和既有`direct@1`仍不声明Memory节点；只有显式Memory运行模式才准备/启动Sidecar，且只有用户选择独立Memory Workflow或确认Session导入才调用Memory。双Provider评测、Memory Agent和管理表面尚未实现，不能因Memory Direct与Session导入已可运行就宣称完整产品能力已完成。
