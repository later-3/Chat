...
| 移动端布局 | 固定`dsh-mobile-hanui@0.2.4`（MIT）作为DSH profile bundle提供移动端外壳（抽屉/FAB/弹窗全屏/Composer修复）；Chat唯一自建控件是workflow选择器，与外壳共存。合同测试`dsh-mobile-hanui-real.spec.ts` |
