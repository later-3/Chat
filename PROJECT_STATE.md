...
| Product Store | 当前写代为`chat-product-store.v24`。它完整保留v23的Capability与Content Project双谱系事实，只新增监督执行v3的空集合与严格关系门；v23迁移不推断Epoch、Round、Evidence或Review，原子替换与重复打开字节幂等。监督基础合同冻结Run/Epoch/Contract/Step/Round、AgentVersion v2、Prompt Assembly v5和qualified Capability；当前尚未接入Application、Workflow、Pi、API、Bridge或Trace，见[监督执行v24基础](./docs/architecture/supervised-execution-v24-foundation-as-built.md) |
