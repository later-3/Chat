...
def test_idiom_chain_requires_two_approvals_and_records_public_node_content(tmp_path) -> None:
    ...
    with TestClient(app) as client:
        ...
        first = _events(client.post(
            idiom["endpoint"],
            json=_request(session_id, "idiom-start", [
                {"id": "user-1", "role": "user", "content": "一心一意"}
            ]),
        ))
        agent_a = _card(first)
    ...
    content_events = [
        value["payload"] for value in trace
        if value["event_type"] == "workflow.node.content"
    ]
    assert {value["executor_id"] for value in content_events} == {
        "idiom_input", "idiom_agent_a", "idiom_handoff", "idiom_agent_b", "idiom_result"
    }
...
def test_idiom_chain_rejects_broken_continuation_before_provider_call(tmp_path) -> None:
    ...
    with TestClient(app) as client:
        ...
        first = _card(_events(client.post(
            ...
            json=_request(session_id, "valid-start", [
...
def test_abandon_second_idiom_agent_keeps_one_dispatch_and_no_fake_answer(tmp_path) -> None:
    ...
    with TestClient(app) as client:
        ...
        first = _card(_events(client.post(
            "/api/workflows/governed-idiom-chain/run",
            json=_request(session_id, "start", [
                {"id": "user", "role": "user", "content": "一心一意"}
            ]),
        )))
        second = _card(_events(client.post(
            "/api/workflows/governed-idiom-chain/run",
