...
def _text(value: str, *, field: str, max_length: int | None = None) -> str:
...
def _project_view(value: ProductProjectRecord) -> dict[str, Any]:
    return {
        "id": value.id,
...
def _work_view(value: WorkItemRecord) -> dict[str, Any]:
    return {
        "id": value.id,
        "scope_id": value.scope_id,
...
def _note_view(value: NoteRecord, revision: NoteRevisionRecord | None) -> dict[str, Any]:
    return {
        "id": value.id,
        "scope_id": value.scope_id,
        "kind": value.kind,
...
def _memory_view(value: AcceptedMemoryRecord, revision: MemoryRevisionRecord | None) -> dict[str, Any]:
    return {
        "id": value.id,
        "scope_id": value.scope_id,
...
def _action_view(value: ActionItemRecord) -> dict[str, Any]:
    return {
        "id": value.id,
        "scope_id": value.scope_id,
        "project_id": value.project_id,
...
class HarnessService:
    ...
    def _record_command(
        ...
        *,
        ...
        now = self._clock()
        transaction.add(HarnessCommandRecord(
            id=_uuid(), scope_id=self.scope_id, command_id=command_id,
            command_kind=command_kind, principal_id=self.principal_id,
            request_hash=request_hash, result_json=dict(result),
            decision_record_id=decision_record_id,
            created_at=now,
        ))
        transaction.add(HarnessTraceRecord(
            id=_uuid(), scope_id=self.scope_id, command_id=command_id,
            event_type=event_type, resource_kind=resource_kind,
            resource_id=resource_id, payload_json=dict(trace_payload),
            created_at=now,
        ))
        transaction.add(GovernanceOutboxRecord(
            id=_uuid(), aggregate_kind=resource_kind, aggregate_id=resource_id,
            event_type=event_type,
            payload_json={"scope_id": self.scope_id, "command_id": command_id, **dict(trace_payload)},
            dedupe_key=f"harness:{self.scope_id}:{command_id}", status="pending",
            available_at=now, attempt_count=0, created_at=now,
        ))
    ...
    async def create_project(
        ...
        *,
        ...
        payload = {"kind": kind, "title": _text(title, field="Project标题", max_length=180),
                   "goal": _text(goal, field="Project目标"), "status": status, "session_id": session_id}
        ...
        async with self.database.sessions.begin() as transaction:
            ...
            value = ProductProjectRecord(
                id=_uuid(), scope_id=self.scope_id, kind=kind, title=payload["title"],
                goal=payload["goal"], status=status, row_version=1,
                created_by=self.principal_id, created_at=self._clock(), updated_at=self._clock(),
            )
            transaction.add(value)
            if session_id:
                transaction.add(ResourceSessionLinkRecord(
                    id=_uuid(), resource_kind="project", resource_id=value.id,
                    session_id=session_id, reason="创建Project时关联的Product Session",
                    source_kind="explicit_user_command",
                ))
            result = _project_view(value)
            self._record_command(
                transaction, command_id=command_id, command_kind="create_project",
                request_hash=request_hash, result=result, resource_kind="project",
                resource_id=value.id, event_type="harness.project.created",
    ...
    async def transition_project(
        ...
        *,
        ...
        request = {"project_id": project_id, "expected_row_version": expected_row_version,
        ...
        async with self.database.sessions.begin() as transaction:
            ...
            if target_status == "completed":
                open_count = await transaction.scalar(
                    select(func.count()).select_from(WorkItemRecord).where(
            ...
            result = _project_view(value)
            self._record_command(
                transaction, command_id=command_id, command_kind="transition_project",
                request_hash=request_hash, result=result, resource_kind="project",
                resource_id=value.id, event_type="harness.project.transitioned",
    ...
    async def create_work_item(
        ...
        *,
        ...
        payload = {
            "kind": kind, "title": _text(title, field="Work标题", max_length=200),
            "objective": _text(objective, field="Work目标"), "project_id": project_id,
            "parent_work_item_id": parent_work_item_id, "priority": priority, "status": status,
        }
        request_hash = _hash(payload)
        async with self.database.sessions.begin() as transaction:
            existing = await self._existing_command(transaction, command_id, request_hash)
            ...
            value = WorkItemRecord(
                id=_uuid(), scope_id=self.scope_id, project_id=project_id,
                parent_work_item_id=parent_work_item_id, kind=kind,
                title=payload["title"], objective=payload["objective"], status=status,
                priority=priority, completion_evidence_json=[], row_version=1,
                created_by=self.principal_id, created_at=self._clock(), updated_at=self._clock(),
            )
            transaction.add(value)
            if project_id:
                ...
                transaction.add(ProjectWorkLinkRecord(
                    id=_uuid(), project_id=project_id, work_item_id=value.id,
                    relation="contains", created_at=self._clock(),
                ))
            result = _work_view(value)
            self._record_command(
                transaction, command_id=command_id, command_kind="create_work_item",
                request_hash=request_hash, result=result, resource_kind="work_item",
                resource_id=value.id, event_type="harness.work.created",
    ...
    async def transition_work_item(
        ...
        *,
        ...
        reason = _text(reason, field="状态变更原因")
        request = {
            "work_item_id": work_item_id, "expected_row_version": expected_row_version,
            "target_status": target_status, "reason": reason, "evidence": list(evidence),
        ...
        async with self.database.sessions.begin() as transaction:
            ...
            result = _work_view(value)
            self._record_command(
                transaction, command_id=command_id, command_kind="transition_work_item",
                request_hash=request_hash, result=result, resource_kind="work_item",
                resource_id=value.id, event_type="harness.work.transitioned",
                trace_payload={"from": previous, "to": target_status, "reason": reason,
        ...
        *,
        ...
        for ordinal, raw in enumerate(nodes):
            ...
            normalized_nodes.append({
                ...
                "objective": _text(str(raw.get("objective") or raw.get("title") or ""), field="Plan节点目标"),
        ...
        dependencies_by_key = {
            str(node["key"]): tuple(str(value) for value in node["dependencies"])
            for node in normalized_nodes
        }
        ...
        for node_key in dependencies_by_key:
            visit(node_key)
        request = {
            "work_item_id": work_item_id, "expected_work_row_version": expected_work_row_version,
            "summary": _text(summary, field="Plan摘要"), "nodes": normalized_nodes,
            "validation_contract": dict(validation_contract or {}), "accept": accept,
        }
        request_hash = _hash(request)
        async with self.database.sessions.begin() as transaction:
            existing = await self._existing_command(transaction, command_id, request_hash)
            ...
            if work.row_version != expected_work_row_version:
                raise HarnessConflict("WorkItem版本冲突")
            plan = await transaction.scalar(select(TaskPlanRecord).where(TaskPlanRecord.work_item_id == work_item_id))
            if plan is None:
                plan = TaskPlanRecord(
                    id=_uuid(), scope_id=self.scope_id, project_id=work.project_id,
                    work_item_id=work.id, status="draft", row_version=1,
                    created_by=self.principal_id, created_at=self._clock(), updated_at=self._clock(),
                )
                ...
                revision_no = int(await transaction.scalar(
            ...
            revision = TaskPlanRevisionRecord(
                id=_uuid(), task_plan_id=plan.id, revision=revision_no,
                previous_revision_id=previous_revision_id, summary=request["summary"],
                validation_contract_json=request["validation_contract"],
                status="accepted" if accept else "candidate", created_by=self.principal_id,
            ...
            for ordinal, node in enumerate(normalized_nodes):
                record = PlanNodeRecord(
                    id=_uuid(), plan_revision_id=revision.id, node_key=node["key"],
                    title=node["title"], objective=node["objective"], status="pending",
                    assignee_kind=node["assignee_kind"], dependency_keys_json=node["dependencies"],
                    validation_json=node["validation"], stop_condition=node["stop_condition"], ordinal=ordinal,
                )
            ...
            result = {
                "id": plan.id, "work_item_id": work.id, "status": plan.status,
                "row_version": plan.row_version,
                "revision": {"id": revision.id, "revision": revision.revision,
                             "summary": revision.summary, "status": revision.status,
                             "nodes": result_nodes},
            }
            self._record_command(
                transaction, command_id=command_id, command_kind="create_plan_revision",
                request_hash=request_hash, result=result, resource_kind="task_plan",
                resource_id=plan.id, event_type="harness.plan.revised",
    ...
    async def get_work_item(self, work_item_id: str) -> dict[str, Any]:
        async with self.database.sessions() as transaction:
            ...
            if plan is not None and plan.current_revision_id:
                ...
                nodes = (await transaction.scalars(
                    select(PlanNodeRecord).where(
                        PlanNodeRecord.plan_revision_id == plan.current_revision_id
            ...
            actions = (await transaction.scalars(
        ...
        *,
        ...
        async with self.database.sessions.begin() as transaction:
            ...
            value = ActionItemRecord(
                id=_uuid(), scope_id=self.scope_id, project_id=project_id,
                work_item_id=work_item_id, plan_node_id=plan_node_id,
                title=request["title"], assignee_kind=assignee_kind, status=status,
                due_at=due_at, evidence_json=[], row_version=1,
                created_at=now, updated_at=now,
            )
            transaction.add(value)
            result = _action_view(value)
            self._record_command(
                transaction, command_id=command_id, command_kind="create_action_item",
                request_hash=request_hash, result=result, resource_kind="action_item",
                resource_id=value.id, event_type="harness.action.created",
                trace_payload={"status": status, "assignee_kind": assignee_kind,
                               "work_item_id": work_item_id},
                decision_record_id=decision_record_id,
            )
            return result
        ...
        async with self.database.sessions.begin() as transaction:
            ...
            if target_status == "ready" and value.plan_node_id:
                ...
                if dependencies and node is not None:
                    dependency_nodes = (await transaction.scalars(
                        select(PlanNodeRecord).where(
                    ...
                    if dependency_ids:
                        complete_ids = set((await transaction.scalars(
                    ...
                    if unresolved and not request["dependency_override_reason"]:
                        raise HarnessValidationError(
                            f"依赖未完成，不能进入ready: {sorted(unresolved)}"
                        )
            ...
            result = _action_view(value)
            self._record_command(
                transaction, command_id=command_id, command_kind="transition_action_item",
                request_hash=request_hash, result=result, resource_kind="action_item",
                resource_id=value.id, event_type="harness.action.transitioned",
                trace_payload={"from": previous, "to": target_status, "reason": reason,
    ...
    async def capture_note(
        ...
        *,
        ...
        request = {"kind": kind, "title": _text(title, field="Note标题", max_length=220),
                   "content": _text(content, field="Note内容"),
                   "source_refs": [dict(item) for item in source_refs],
                   "links": [dict(item) for item in links], "status": status}
        request_hash = _hash(request)
        async with self.database.sessions.begin() as transaction:
            existing = await self._existing_command(transaction, command_id, request_hash)
            if existing is not None:
                return existing
            note = NoteRecord(
                id=_uuid(), scope_id=self.scope_id, kind=kind, title=request["title"],
                status=status, row_version=1, created_by=self.principal_id,
                created_at=self._clock(), updated_at=self._clock(),
            )
            revision = NoteRevisionRecord(
                id=_uuid(), note_id=note.id, revision=1, previous_revision_id=None,
                content=request["content"], source_refs_json=request["source_refs"],
                content_hash=_hash(request["content"]), created_by=self.principal_id,
                created_at=self._clock(),
            )
            ...
            for link in request["links"]:
                resource_kind = _text(str(link.get("resource_kind") or ""), field="Note关联资源类型", max_length=40)
                resource_id = _text(str(link.get("resource_id") or ""), field="Note关联资源ID", max_length=100)
                transaction.add(NoteResourceLinkRecord(
                    id=_uuid(), note_id=note.id, resource_kind=resource_kind,
                    resource_id=resource_id, relation=str(link.get("relation") or "documents"),
                    created_at=self._clock(),
                ))
            result = _note_view(note, revision)
            self._record_command(
                transaction, command_id=command_id, command_kind="capture_note",
                request_hash=request_hash, result=result, resource_kind="note",
                resource_id=note.id, event_type="harness.note.created",
    ...
    async def revise_note(
        ...
        *,
        ...
        request = {"note_id": note_id, "expected_row_version": expected_row_version,
        ...
        async with self.database.sessions.begin() as transaction:
            ...
            revision = NoteRevisionRecord(
                id=_uuid(), note_id=note.id, revision=revision_no,
                previous_revision_id=previous_id, content=request["content"],
                source_refs_json=request["source_refs"], content_hash=_hash(request["content"]),
                created_by=self.principal_id, created_at=self._clock(),
            )
            transaction.add(revision)
            ...
            result = _note_view(note, revision)
            self._record_command(
                transaction, command_id=command_id, command_kind="revise_note",
                request_hash=request_hash, result=result, resource_kind="note",
                resource_id=note.id, event_type="harness.note.revised",
        ...
        *,
        ...
        async with self.database.sessions.begin() as transaction:
            ...
            if target_status in {"superseded", "archived"}:
                ...
                memories = (await transaction.scalars(
                ...
                candidates = (await transaction.scalars(
            ...
            revision = (
                ...
                if note.current_revision_id else None
            )
            result = _note_view(note, revision)
            self._record_command(
                transaction, command_id=command_id, command_kind="transition_note",
                request_hash=request_hash, result=result, resource_kind="note",
                resource_id=note.id, event_type="harness.note.transitioned",
                trace_payload={
                    "from": previous,
                    "to": target_status,
                    "reason": reason,
    ...
    async def list_notes(self, *, project_id: str | None = None, limit: int = 200) -> list[dict[str, Any]]:
        async with self.database.sessions() as transaction:
            ...
            notes = (await transaction.scalars(
                query.order_by(NoteRecord.updated_at.desc()).limit(min(max(limit, 1), 500))
            )).all()
            result: list[dict[str, Any]] = []
            for note in notes:
                revision = await transaction.get(NoteRevisionRecord, note.current_revision_id) if note.current_revision_id else None
    ...
    async def propose_memory(
        ...
        *,
        ...
        request = {"scope_kind": scope_kind, "scope_ref_id": scope_ref_id,
                   "memory_kind": memory_kind, "content": _text(content, field="Memory内容"),
                   "source_refs": [dict(item) for item in source_refs]}
        request_hash = _hash(request)
        async with self.database.sessions.begin() as transaction:
            existing = await self._existing_command(transaction, command_id, request_hash)
            ...
            candidate = MemoryCandidateRecord(
                id=_uuid(), scope_id=self.scope_id, scope_kind=scope_kind,
                scope_ref_id=scope_ref_id, memory_kind=memory_kind,
                content=request["content"], status="pending_review",
                source_refs_json=request["source_refs"], content_hash=_hash(request["content"]),
                proposed_by=self.principal_id, decision_record_id=decision_record_id,
                created_at=self._clock(),
            )
            transaction.add(candidate)
            result = {
                "id": candidate.id, "scope_kind": candidate.scope_kind,
                "scope_ref_id": candidate.scope_ref_id, "memory_kind": candidate.memory_kind,
                "content": candidate.content, "status": candidate.status,
                "source_refs": candidate.source_refs_json, "content_hash": candidate.content_hash,
                "created_at": _iso(candidate.created_at),
            }
            self._record_command(
                transaction, command_id=command_id, command_kind="propose_memory",
                request_hash=request_hash, result=result, resource_kind="memory_candidate",
                resource_id=candidate.id, event_type="harness.memory.proposed",
    ...
    async def resolve_memory_candidate(
        ...
        *,
        ...
        request = {"candidate_id": candidate_id, "decision": decision,
                   "decision_record_id": decision_record_id}
        request_hash = _hash(request)
        async with self.database.sessions.begin() as transaction:
            existing = await self._existing_command(transaction, command_id, request_hash)
            ...
            if decision != "accept":
                ...
                event_type = "harness.memory.rejected" if decision == "reject" else "harness.memory.session_only"
                resource_kind = "memory_candidate"
                resource_id = candidate.id
            else:
                candidate.status = "accepted"
                memory = AcceptedMemoryRecord(
                    id=_uuid(), scope_id=self.scope_id, scope_kind=candidate.scope_kind,
                    scope_ref_id=candidate.scope_ref_id, memory_kind=candidate.memory_kind,
                    status="accepted", row_version=1,
                    created_at=self._clock(), updated_at=self._clock(),
                )
                revision = MemoryRevisionRecord(
                    id=_uuid(), memory_id=memory.id, candidate_id=candidate.id, revision=1,
                    previous_revision_id=None, content=candidate.content,
                    content_hash=candidate.content_hash, source_refs_json=candidate.source_refs_json,
                    decision_record_id=decision_record_id, created_by=self.principal_id,
                    created_at=self._clock(),
                )
                ...
                for source in candidate.source_refs_json or []:
                    if not isinstance(source, Mapping) or not source.get("id"):
                        continue
                    transaction.add(MemorySourceLinkRecord(
                        id=_uuid(), memory_revision_id=revision.id,
            ...
            self._record_command(
                transaction, command_id=command_id, command_kind="resolve_memory_candidate",
                request_hash=request_hash, result=result, resource_kind=resource_kind,
        ...
        *,
        ...
        async with self.database.sessions.begin() as transaction:
            ...
            previous = (
                ...
                if memory.current_revision_id else None
            )
            revision = MemoryRevisionRecord(
                id=_uuid(), memory_id=memory.id, candidate_id=None,
                revision=(previous.revision if previous else 0) + 1,
                previous_revision_id=memory.current_revision_id,
                content=request["content"], content_hash=_hash(request["content"]),
            ...
            self._record_command(
                transaction, command_id=command_id, command_kind="revise_memory",
                request_hash=request_hash, result=result, resource_kind="accepted_memory",
                resource_id=memory.id, event_type="harness.memory.revised",
                trace_payload={"revision": revision.revision, "reason": request["reason"],
        ...
        *,
        ...
        async with self.database.sessions.begin() as transaction:
            ...
            if target_status not in MEMORY_TRANSITIONS.get(memory.status, set()):
                raise HarnessValidationError(
                    f"Memory不能从{memory.status}变为{target_status}"
                )
            ...
            result = _memory_view(memory, revision)
            self._record_command(
                transaction, command_id=command_id, command_kind="transition_memory",
                request_hash=request_hash, result=result, resource_kind="accepted_memory",
                resource_id=memory.id, event_type="harness.memory.transitioned",
                trace_payload={"from": previous_status, "to": target_status,
    ...
    async def list_memory(
        ...
        *,
        ...
        async with self.database.sessions() as transaction:
            ...
            for memory in memories:
                revision = await transaction.get(MemoryRevisionRecord, memory.current_revision_id) if memory.current_revision_id else None
                accepted.append(_memory_view(memory, revision))
            candidates: list[dict[str, Any]] = []
            if include_candidates:
                candidate_query = select(MemoryCandidateRecord).where(
                    MemoryCandidateRecord.scope_id == self.scope_id,
                    MemoryCandidateRecord.status.in_(("candidate", "pending_review")),
                ).order_by(MemoryCandidateRecord.created_at.desc()).limit(min(max(limit, 1), 500))
                for value in (await transaction.scalars(candidate_query)).all():
                    candidates.append({
                        "id": value.id, "scope_kind": value.scope_kind,
                        "scope_ref_id": value.scope_ref_id, "memory_kind": value.memory_kind,
    ...
    async def search_resources(self, query: str, *, limit: int = 30) -> list[dict[str, Any]]:
        ...
        async with self.database.sessions() as transaction:
            projects = (await transaction.scalars(select(ProductProjectRecord).where(
                ProductProjectRecord.scope_id == self.scope_id,
                or_(ProductProjectRecord.title.ilike(pattern), ProductProjectRecord.goal.ilike(pattern)),
            ).limit(limit))).all()
            results.extend({"kind": "project", "id": v.id, "title": v.title,
                            "summary": v.goal, "status": v.status, "revision": v.row_version} for v in projects)
            work = (await transaction.scalars(select(WorkItemRecord).where(
                WorkItemRecord.scope_id == self.scope_id,
                or_(WorkItemRecord.title.ilike(pattern), WorkItemRecord.objective.ilike(pattern)),
            ).limit(limit))).all()
            results.extend({"kind": "work_item", "id": v.id, "title": v.title,
                            "summary": v.objective, "status": v.status, "revision": v.row_version} for v in work)
            notes = (await transaction.execute(
                select(NoteRecord, NoteRevisionRecord).join(
                    NoteRevisionRecord, NoteRevisionRecord.id == NoteRecord.current_revision_id
                ).where(NoteRecord.scope_id == self.scope_id,
                        or_(NoteRecord.title.ilike(pattern), NoteRevisionRecord.content.ilike(pattern))).limit(limit)
            )).all()
            results.extend({"kind": "note", "id": note.id, "title": note.title,
                            "summary": revision.content[:400], "status": note.status,
                            "revision": revision.revision} for note, revision in notes)
        return results[:limit]

    async def project_context(self, project_id: str) -> dict[str, Any]:
        project = await self.get_project(project_id)
        work = await self.list_work(project_id=project_id)
        work_details = [await self.get_work_item(value["id"]) for value in work]
        actions = await self.list_action_items(project_id=project_id)
        notes = await self.list_notes(project_id=project_id)
        memory = await self.list_memory(
            scope_kind="project",
        ...
        async with self.database.sessions.begin() as transaction:
            ...
            for ordinal, raw in enumerate(items):
                ...
                if adopted:
                    total += estimate
                normalized.append({
                    "ordinal": ordinal, "source_kind": str(raw.get("source_kind") or "unknown"),
                    "source_id": str(raw.get("source_id") or "unknown"),
                    "source_revision": str(raw.get("source_revision")) if raw.get("source_revision") is not None else None,
                    "title": str(raw.get("title") or raw.get("source_kind") or "未命名来源"),
                    "content": content, "adopted": adopted, "reason": reason,
            ...
            if existing is not None:
                return existing
            revision = int(await transaction.scalar(
                select(func.max(ContextPackageRecord.revision)).where(
                    ContextPackageRecord.run_id == run_id,
                    ContextPackageRecord.stage == stage,
                )
            ) or 0) + 1
            package_hash = _hash({"stage": stage, "project": selected_project_id,
                                  "work": selected_work_item_id, "items": normalized})
            now = self._clock()
            package = ContextPackageRecord(
                id=_uuid(), scope_id=self.scope_id, session_id=session_id,
                interaction_id=run.interaction_id, run_id=run_id, stage=stage,
                revision=revision, selected_project_id=selected_project_id,
                selected_work_item_id=selected_work_item_id, token_budget=token_budget,
                estimated_tokens=total, package_hash=package_hash, status=status,
                created_at=now,
            )
            transaction.add(package)
            await transaction.flush()
            for item in normalized:
                transaction.add(ContextAdoptionRecord(
                    id=_uuid(), context_package_id=package.id, ordinal=item["ordinal"],
                    source_kind=item["source_kind"], source_id=item["source_id"],
                    source_revision=item["source_revision"], title=item["title"],
                    content_text=item["content"], adopted=item["adopted"],
                    reason=item["reason"], token_estimate=item["token_estimate"],
                ))
            result = {
                "id": package.id, "session_id": session_id, "run_id": run_id,
                "stage": stage, "revision": revision,
                "selected_project_id": selected_project_id,
                "selected_work_item_id": selected_work_item_id,
                "token_budget": token_budget, "estimated_tokens": total,
                "package_hash": package_hash, "status": status, "items": normalized,
            }
            self._record_command(
                transaction,
    ...
    async def latest_context_package(self, session_id: str) -> dict[str, Any] | None:
        async with self.database.sessions() as transaction:
            package = await transaction.scalar(select(ContextPackageRecord).where(
                ContextPackageRecord.session_id == session_id,
                ContextPackageRecord.scope_id == self.scope_id,
            ).order_by(ContextPackageRecord.created_at.desc()).limit(1))
            if package is None:
                return None
            items = (await transaction.scalars(select(ContextAdoptionRecord).where(
                ContextAdoptionRecord.context_package_id == package.id
            ).order_by(ContextAdoptionRecord.ordinal))).all()
            return {
                "id": package.id, "session_id": package.session_id, "run_id": package.run_id,
                "stage": package.stage, "revision": package.revision,
                "selected_project_id": package.selected_project_id,
                "selected_work_item_id": package.selected_work_item_id,
                "token_budget": package.token_budget, "estimated_tokens": package.estimated_tokens,
                "package_hash": package.package_hash, "status": package.status,
                "created_at": _iso(package.created_at),
                "items": [{
                    "source_kind": item.source_kind, "source_id": item.source_id,
                    "source_revision": item.source_revision, "title": item.title,
                    "content": item.content_text, "adopted": item.adopted,
                    "reason": item.reason, "token_estimate": item.token_estimate,
                } for item in items],
            }
    ...
    async def directory_context_items(
        ...
        *,
        ...
        projects = await self.list_projects(statuses=("proposed", "active", "paused"), limit=max_projects)
        ...
        directory_projects = [
            ...
            for score, value in ranked[:8]
        ]
        directory = [{
            "source_kind": "project_directory", "source_id": value["id"],
            "source_revision": value["row_version"], "title": value["title"],
            "content": _canonical({"kind": value["kind"], "status": value["status"], "goal": value["goal"]}),
            "adopted": True,
            "reason": (
                "Project轻量目录与当前输入匹配"
                if value["id"] in matched_project_ids else "最近更新的正式Project轻量目录"
            ),
        } for value in directory_projects]
        directory.extend({
            "source_kind": "turn_summary", "source_id": str(value.get("id") or "unknown"),
            "source_revision": value.get("summary_hash"), "title": str(value.get("topic") or "回合主题"),
            "content": _canonical(value.get("summary") or value), "adopted": True,
    ...
    async def detailed_context_items(self, project_id: str) -> list[dict[str, Any]]:
        ...
        items: list[dict[str, Any]] = [{
            ...
            "source_revision": project["row_version"], "title": project["title"],
            "content": _canonical({"goal": project["goal"], "status": project["status"], "kind": project["kind"]}),
            "adopted": True, "reason": "用户或意图已绑定该Project",
        }]
        for work in context["work_items"]:
            if work["status"] in {"archived", "cancelled"}:
                continue
            items.append({
                "source_kind": "work_item", "source_id": work["id"],
                "source_revision": work["row_version"], "title": work["title"],
                "content": _canonical({"objective": work["objective"], "status": work["status"],
                                       "priority": work["priority"], "evidence": work["completion_evidence"]}),
                "adopted": True, "reason": "绑定Project中的开放Work",
            })
        ...
        for note in context["notes"][:8]:
            ...
            items.append({
                "source_kind": "note", "source_id": note["id"],
                "source_revision": revision.get("revision"), "title": note["title"],
                "content": str(revision.get("content") or ""), "adopted": True,
                "reason": "与绑定Project显式关联的当前Note revision",
            })
        for memory in context["accepted_memory"]:
            revision = memory.get("current_revision") or {}
            items.append({
                "source_kind": "accepted_memory", "source_id": memory["id"],
                "source_revision": revision.get("revision"), "title": memory["memory_kind"],
                "content": str(revision.get("content") or ""), "adopted": True,
    ...
    async def learning_tracks(self) -> list[dict[str, Any]]:
        projects = await self.list_projects(kind="learning")
        tracks: list[dict[str, Any]] = []
        for project in projects:
            units = await self.list_work(project_id=project["id"])
            tracks.append({
                "project": project,
    ...
    async def commit_turn_candidates(
        ...
        *,
        ...
        async with self.database.sessions.begin() as transaction:
            ...
            for index, raw in enumerate(work_candidates):
                ...
                work = WorkItemRecord(
                    id=_uuid(), scope_id=self.scope_id, project_id=project_id,
                    parent_work_item_id=None, kind=kind, title=title[:200],
                    objective=str(candidate.get("objective") or title), status=requested_status,
                    priority=str(candidate.get("priority") or "normal")
                    if str(candidate.get("priority") or "normal") in WORK_PRIORITIES else "normal",
                    completion_evidence_json=list(evidence or []), row_version=1,
                ...
                if project_id:
                    transaction.add(ProjectWorkLinkRecord(
                        id=_uuid(), project_id=project_id, work_item_id=work.id,
                        relation="contains", created_at=self._clock(),
                    ))
                transaction.add(ResourceSessionLinkRecord(
                    id=_uuid(), resource_kind="work_item", resource_id=work.id,
                    session_id=session_id, reason="本轮Work候选经决定点提交",
                    source_kind="turn_summary_candidate",
                ))
                committed_work.append(_work_view(work))
            for index, raw in enumerate(memory_candidates):
                ...
                if not content:
                    continue
                memory_kind = str(candidate_data.get("memory_kind") or candidate_data.get("kind") or "stable_fact")
                if memory_kind not in MEMORY_KINDS:
                    memory_kind = "stable_fact"
                scope_kind = "project" if project_id else "user"
                source_refs = [{"kind": "product_run", "id": run_id},
                               {"kind": "product_session", "id": session_id}]
                candidate = MemoryCandidateRecord(
                    id=_uuid(), scope_id=self.scope_id, scope_kind=scope_kind,
                    scope_ref_id=project_id, memory_kind=memory_kind, content=content,
                    status="accepted", source_refs_json=source_refs,
                    content_hash=_hash(content), proposed_by="turn_summary_agent",
                    decision_record_id=decision_record_ids[-1] if decision_record_ids else None,
                    created_at=self._clock(), resolved_at=self._clock(),
                )
                memory = AcceptedMemoryRecord(
                    id=_uuid(), scope_id=self.scope_id, scope_kind=scope_kind,
                    scope_ref_id=project_id, memory_kind=memory_kind, status="accepted",
                    row_version=1, created_at=self._clock(), updated_at=self._clock(),
                )
                revision = MemoryRevisionRecord(
                    id=_uuid(), memory_id=memory.id, candidate_id=candidate.id,
                    revision=1, previous_revision_id=None, content=content,
                    content_hash=candidate.content_hash, source_refs_json=source_refs,
                ...
                for source in source_refs:
                    transaction.add(MemorySourceLinkRecord(
                        id=_uuid(), memory_revision_id=revision.id,
                        source_kind=source["kind"], source_id=source["id"], source_revision=None,
                        created_at=self._clock(),
                    ))
            ...
            self._record_command(
                transaction, command_id=command_id, command_kind="commit_turn_candidates",
                request_hash=request_hash, result=result, resource_kind="product_run",
                resource_id=run_id, event_type="harness.turn_candidates.committed",
                trace_payload={"work_count": len(committed_work),
                               "memory_count": len(committed_memory),
                               "project_id": project_id},
                decision_record_id=decision_record_ids[-1] if decision_record_ids else None,
            )
