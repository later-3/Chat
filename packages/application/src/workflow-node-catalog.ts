...
export const NODE_CATALOG_DESCRIPTORS: readonly NodeCatalogDescriptor[] = [
  ...
  },
  {
    ...
    schemaVersion: 1,
    displayName: "检查工程规范",
    description: "依据冻结执行候选、证据和本次选中规范产生结构化采用门检查",
    category: "validation",
    configSchema: z.strictObject({
      strictEvidence: z.boolean().default(true),
      agentKey: z.literal("governance_reviewer").optional(),
      agentPromptOverride: z.string().max(65_536).optional(),
    }),
    defaultConfig: { strictEvidence: true },
    publicConfigFields: [
      agentKeyField("governance_reviewer", ["governance_reviewer"]),
      agentPromptOverrideField(),
      booleanField("strictEvidence", "缺少必要证据时阻断采用", true),
    ],
