import { TRACE_EVENT_NAMES } from "./trace.js";

/**
 * 每种Trace事件的合法Fixture。仅用于测试与本地调试证据；
 * 全部为合成数据，不含正文、密钥或真实Provider Payload。
 */

const SHA256_A = "a".repeat(64);
const SHA256_B = "b".repeat(64);
const SHA256_C = "c".repeat(64);

const base = {
  schemaVersion: 1,
  eventId: "evt_fx1",
  timestamp: "2026-08-07T00:00:00.000Z",
  level: "info",
  traceId: "trace_fx1",
  spanId: "span_fx1",
};

const run = { productRunId: "run_fx1", attemptId: "att_fx1" };
const wfd = { ...run, workflowDefinitionVersion: "1.0.0" };
const model = {
  ...run,
  promptTemplateVersion: "planner-1.0.0",
  modelConfigVersion: "bailian-qwen-1.0.0",
};
const session = { productSessionId: "psn_fx1", interactionId: "ixn_fx1" };
const planRef = { objectType: "plan", objectId: "plan_fx1", revision: 2, sha256: SHA256_A };
const decisionRef = { objectType: "decision", objectId: "dec_fx1", revision: 1, sha256: SHA256_B };
const candidateRef = { objectType: "execution_candidate", objectId: "exc_fx1", sha256: SHA256_C };
const messageRef = { objectType: "message", objectId: "msg_fx1", sha256: SHA256_A };
const contextPackageRef = {
  objectType: "context_package",
  objectId: "ctxp_fx1",
  revision: 1,
  sha256: SHA256_C,
};
const err = { code: "provider.timeout", type: "TimeoutError" };
const providerShared = {
  provider: "bailian",
  model: "qwen3.7-plus",
  endpointHost: "dashscope.aliyuncs.com",
  operation: "chat_completion",
};
const piOperation = {
  ...model,
  piOperationId: "pio_fx1",
  operationEventSequence: 1,
  sourceTimestamp: "2026-08-07T00:00:00.000Z",
};
const piRuntimeSession = { ...piOperation, piRuntimeSessionId: "pis_fx1" };
const memoryImport = {
  memoryImportIntentId: "mii_fx1",
  memoryImportResultId: "mir_fx1",
  outboxId: "obx_fx1",
  operationId: "mii_fx1",
  backendId: "mbk_memmy",
  requestSha256: SHA256_A,
  intentRevision: 1,
};

export const validTraceFixtures: ReadonlyArray<Record<string, unknown>> = [
  // HTTP
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.httpCommandReceived,
    outcome: "unknown",
    requestId: "req_fx1",
    httpMethod: "POST",
    ...session,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.httpCommandAccepted,
    outcome: "success",
    requestId: "req_fx1",
    httpMethod: "POST",
    routeTemplate: "/api/sessions/:sessionId/messages",
    statusCode: 202,
    ...session,
    productRunId: "run_fx1",
    commandId: "cmd_fx1",
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.httpCommandRejected,
    outcome: "rejected",
    requestId: "req_fx1",
    httpMethod: "POST",
    statusCode: 409,
    errorCode: "revision_conflict",
    commandId: "cmd_fx1",
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.httpCommandCompleted,
    outcome: "success",
    requestId: "req_fx1",
    httpMethod: "GET",
    routeTemplate: "/api/runs/:productRunId",
    statusCode: 200,
    durationMs: 4,
  },
  // 产品事务
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.productTransactionStarted,
    outcome: "unknown",
    transactionType: "message.send",
    ...session,
    commandId: "cmd_fx1",
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.productTransactionCommitted,
    outcome: "success",
    transactionType: "message.send",
    ...session,
    commandId: "cmd_fx1",
    productRunId: "run_fx1",
    inputRefs: [messageRef],
    outputRefs: [messageRef],
    durationMs: 7,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.productTransactionFailed,
    outcome: "failure",
    transactionType: "decision.commit",
    commandId: "cmd_fx1",
    error: { code: "revision_conflict", type: "ConflictError" },
  },
  // Product Run
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.productRunCreated,
    outcome: "success",
    productRunId: "run_fx1",
    ...session,
    runStatus: "planning",
    phase: "plan",
    revision: 0,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.productRunTransitioned,
    outcome: "success",
    productRunId: "run_fx1",
    fromStatus: "planning",
    toStatus: "awaiting_decision",
    fromPhase: "plan",
    toPhase: "approval",
    revision: 1,
  },
  // Context / Memory
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.contextAssemblyStarted,
    outcome: "unknown",
    ...run,
    contextRequestId: "ctxr_fx1",
    memoryRequested: true,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.contextAssemblyCompleted,
    outcome: "success",
    ...run,
    contextRequestId: "ctxr_fx1",
    status: "ready",
    memoryRequested: true,
    adoptedCount: 1,
    excludedCount: 0,
    contextPackageRef,
    durationMs: 12,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.contextAssemblyFailed,
    outcome: "failure",
    ...run,
    contextRequestId: "ctxr_fx1",
    memoryRequested: true,
    error: { code: "memory.backend.timeout", type: "MemoryBackendError" },
    durationMs: 10000,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.memoryQueryStarted,
    outcome: "unknown",
    ...run,
    contextRequestId: "ctxr_fx1",
    memoryQueryId: "mqy_fx1",
    backendId: "mbk_memmy",
    requirement: "optional",
    sourceMessageSha256: SHA256_A,
    tagCount: 1,
    layerCount: 1,
    requestedLimit: 3,
    contextBudget: 512,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.memoryQueryCompleted,
    outcome: "success",
    ...run,
    contextRequestId: "ctxr_fx1",
    memoryQueryId: "mqy_fx1",
    backendId: "mbk_memmy",
    requirement: "optional",
    sourceMessageSha256: SHA256_A,
    tagCount: 1,
    layerCount: 1,
    requestedLimit: 3,
    contextBudget: 512,
    hitCount: 2,
    adoptedCount: 1,
    resultSetSha256: SHA256_B,
    durationMs: 25,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.memoryQueryFailed,
    outcome: "failure",
    ...run,
    contextRequestId: "ctxr_fx1",
    memoryQueryId: "mqy_fx1",
    backendId: "mbk_memmy",
    requirement: "required",
    sourceMessageSha256: SHA256_A,
    tagCount: 1,
    layerCount: 1,
    requestedLimit: 3,
    contextBudget: 512,
    error: { code: "memory.backend.timeout", type: "MemoryBackendError" },
    durationMs: 10000,
  },
  // Memory Import：仅记录身份、Hash、状态和耗时，不复制Message或外部响应正文。
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.memoryImportIntentCreated,
    outcome: "success",
    ...memoryImport,
    memoryImportIntentId: "mii_fx1",
    memoryImportResultId: "mir_fx1",
    operationId: "mii_fx1",
    backendId: "mbk_memmy",
    requestSha256: SHA256_A,
    resultRevision: 1,
    backendDescriptorSha256: SHA256_B,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.memoryImportStarted,
    outcome: "unknown",
    ...memoryImport,
    memoryImportIntentId: "mii_fx1",
    memoryImportResultId: "mir_fx1",
    operationId: "mii_fx1",
    backendId: "mbk_memmy",
    requestSha256: SHA256_A,
    resultRevision: 2,
    dispatchAttempt: 1,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.memoryImportAccepted,
    outcome: "success",
    ...memoryImport,
    memoryImportIntentId: "mii_fx1",
    memoryImportResultId: "mir_fx1",
    operationId: "mii_fx1",
    backendId: "mbk_memmy",
    requestSha256: SHA256_A,
    resultRevision: 3,
    externalObjectIdSha256: SHA256_B,
    responseSha256: SHA256_C,
    dispatchAttempt: 1,
    durationMs: 25,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.memoryImportMaterialized,
    outcome: "success",
    ...memoryImport,
    memoryImportIntentId: "mii_fx1",
    memoryImportResultId: "mir_fx1",
    operationId: "mii_fx1",
    backendId: "mbk_memmy",
    requestSha256: SHA256_A,
    resultRevision: 4,
    externalObjectIdSha256: SHA256_B,
    verificationSha256: SHA256_C,
    reconcileAttempt: 1,
    durationMs: 31,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.memoryImportOutcomeUnknown,
    outcome: "unknown",
    ...memoryImport,
    memoryImportIntentId: "mii_fx1",
    memoryImportResultId: "mir_fx1",
    operationId: "mii_fx1",
    backendId: "mbk_memmy",
    requestSha256: SHA256_A,
    resultRevision: 3,
    origin: "dispatch",
    attempt: 1,
    error: { code: "memory.import.response_lost", type: "MemoryImportBackendError" },
    durationMs: 1000,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.memoryImportFailed,
    outcome: "failure",
    ...memoryImport,
    memoryImportIntentId: "mii_fx1",
    memoryImportResultId: "mir_fx1",
    operationId: "mii_fx1",
    backendId: "mbk_memmy",
    requestSha256: SHA256_A,
    resultRevision: 3,
    origin: "dispatch",
    attempt: 1,
    error: { code: "memory.import.rejected", type: "MemoryImportBackendError" },
    durationMs: 16,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.memoryImportReconcileStarted,
    outcome: "unknown",
    ...memoryImport,
    memoryImportIntentId: "mii_fx1",
    memoryImportResultId: "mir_fx1",
    operationId: "mii_fx1",
    backendId: "mbk_memmy",
    requestSha256: SHA256_A,
    resultRevision: 3,
    reconcileAttempt: 1,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.memoryImportReconcileCompleted,
    outcome: "success",
    ...memoryImport,
    memoryImportIntentId: "mii_fx1",
    memoryImportResultId: "mir_fx1",
    operationId: "mii_fx1",
    backendId: "mbk_memmy",
    requestSha256: SHA256_A,
    resultRevision: 4,
    resolution: "materialized",
    reconcileAttempt: 1,
    externalObjectIdSha256: SHA256_B,
    durationMs: 20,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.memoryImportReconcileFailed,
    outcome: "failure",
    ...memoryImport,
    memoryImportIntentId: "mii_fx1",
    memoryImportResultId: "mir_fx1",
    operationId: "mii_fx1",
    backendId: "mbk_memmy",
    requestSha256: SHA256_A,
    resultRevision: 4,
    reconcileAttempt: 1,
    error: { code: "memory.import.reconcile_failed", type: "MemoryImportBackendError" },
    durationMs: 1000,
  },
  // Project Intake与Resource Observe
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.projectIntakeStarted,
    outcome: "unknown",
    projectCandidateId: "pca_fx1",
    productSessionId: "psn_fx1",
    commandId: "cmd_fx1",
    candidateRevision: 1,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.projectIntakeCandidatePublished,
    outcome: "success",
    projectCandidateId: "pca_fx1",
    candidateRevision: 2,
    candidateSha256: SHA256_A,
    observationSha256: SHA256_B,
    durationMs: 120,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.projectIntakeConfirmed,
    outcome: "success",
    projectCandidateId: "pca_fx1",
    candidateRevision: 3,
    candidateSha256: SHA256_A,
    projectId: "prj_fx1",
    projectRevision: 1,
    commandId: "cmd_fx1",
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.projectIntakeRejected,
    outcome: "rejected",
    projectCandidateId: "pca_fx1",
    candidateRevision: 3,
    candidateSha256: SHA256_A,
    commandId: "cmd_fx1",
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.projectAdvancementStarted,
    outcome: "unknown",
    projectCandidateId: "pca_fx1",
    projectId: "prj_fx1",
    projectStageId: "pst_fx1",
    boundProjectRevision: 2,
    boundStageRevision: 1,
    commandId: "cmd_fx1",
    candidateRevision: 1,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.projectAdvancementCandidatePublished,
    outcome: "success",
    projectCandidateId: "pca_fx1",
    projectId: "prj_fx1",
    projectStageId: "pst_fx1",
    candidateRevision: 2,
    candidateSha256: SHA256_A,
    durationMs: 500,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.projectAdvancementConfirmed,
    outcome: "success",
    projectCandidateId: "pca_fx1",
    projectId: "prj_fx1",
    projectStageId: "pst_fx1",
    projectUpdateId: "pup_fx1",
    candidateRevision: 3,
    candidateSha256: SHA256_A,
    projectRevision: 3,
    stageRevision: 2,
    milestoneCount: 1,
    commandId: "cmd_fx1",
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.projectAdvancementRejected,
    outcome: "rejected",
    projectCandidateId: "pca_fx1",
    projectId: "prj_fx1",
    projectStageId: "pst_fx1",
    candidateRevision: 3,
    candidateSha256: SHA256_A,
    commandId: "cmd_fx1",
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.projectStageTransitioned,
    outcome: "success",
    projectId: "prj_fx1",
    projectStageId: "pst_fx1",
    projectStateTransitionId: "ptr_fx1",
    projectDecisionId: "pdc_fx1",
    fromStatus: "active",
    toStatus: "review",
    beforeRevision: 1,
    afterRevision: 2,
    evidenceCount: 0,
    commandId: "cmd_fx1",
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.projectLifecycleTransitioned,
    outcome: "success",
    projectId: "prj_fx1",
    projectStateTransitionId: "ptr_fx1",
    projectDecisionId: "pdc_fx1",
    fromStatus: "active",
    toStatus: "paused",
    beforeRevision: 2,
    afterRevision: 3,
    evidenceCount: 0,
    commandId: "cmd_fx1",
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.projectMilestoneTransitioned,
    outcome: "success",
    projectId: "prj_fx1",
    projectMilestoneId: "pml_fx1",
    projectStateTransitionId: "ptr_fx1",
    projectDecisionId: "pdc_fx1",
    fromStatus: "planned",
    toStatus: "achieved",
    beforeRevision: 1,
    afterRevision: 2,
    evidenceCount: 1,
    commandId: "cmd_fx1",
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.projectUpdatePublished,
    outcome: "success",
    projectId: "prj_fx1",
    projectStageId: "pst_fx1",
    projectUpdateId: "pup_fx1",
    projectRevision: 3,
    stageRevision: 2,
    updateRevision: 1,
    evidenceCount: 0,
    commandId: "cmd_fx1",
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.projectUnderstandingStarted,
    outcome: "unknown",
    projectCandidateId: "pca_fx1",
    candidateRevision: 1,
    providerName: "bailian",
    modelId: "qwen3.7-plus",
    endpointHost: "dashscope.aliyuncs.com",
    promptTemplateVersion: "project-intake-understanding.v1",
    modelProfileVersion: "bailian.qwen3.7-plus.v1",
    inputManifestSha256: SHA256_A,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.projectUnderstandingCompleted,
    outcome: "success",
    projectCandidateId: "pca_fx1",
    candidateRevision: 1,
    providerName: "bailian",
    modelId: "qwen3.7-plus",
    endpointHost: "dashscope.aliyuncs.com",
    promptTemplateVersion: "project-intake-understanding.v1",
    modelProfileVersion: "bailian.qwen3.7-plus.v1",
    inputManifestSha256: SHA256_A,
    providerRequestId: "req-project-1",
    tokenUsage: { promptTokens: 20, completionTokens: 10, totalTokens: 30 },
    durationMs: 800,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.projectUnderstandingFailed,
    outcome: "failure",
    projectCandidateId: "pca_fx1",
    candidateRevision: 1,
    providerName: "bailian",
    modelId: "qwen3.7-plus",
    endpointHost: "dashscope.aliyuncs.com",
    promptTemplateVersion: "project-intake-understanding.v1",
    modelProfileVersion: "bailian.qwen3.7-plus.v1",
    inputManifestSha256: SHA256_A,
    error: { code: "provider.timeout", type: "ProjectUnderstandingError" },
    durationMs: 90000,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.projectResourceObserveStarted,
    outcome: "unknown",
    projectId: "prj_fx1",
    projectResourceId: "prs_fx1",
    adapterCount: 3,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.projectResourceObserveCompleted,
    outcome: "success",
    projectId: "prj_fx1",
    projectResourceId: "prs_fx1",
    projectObservationId: "pob_fx1",
    observationSha256: SHA256_B,
    adapterCount: 3,
    durationMs: 40,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.projectResourceObserveFailed,
    outcome: "failure",
    projectId: "prj_fx1",
    projectResourceId: "prs_fx1",
    adapterCount: 3,
    error: { code: "project.observe_failed", type: "ProjectResourceError" },
    durationMs: 40,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.projectActionCreated,
    outcome: "success",
    projectId: "prj_fx1",
    projectActionId: "pac_fx1",
    projectWorkId: "pwk_fx1",
    ownerParticipantId: "ppt_fx1",
    actionRevision: 1,
    commandId: "cmd_fx1",
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.projectActionAssigned,
    outcome: "success",
    projectId: "prj_fx1",
    projectActionId: "pac_fx1",
    projectWorkId: "pwk_fx1",
    ownerParticipantId: "ppt_fx1",
    actionRevision: 2,
    commandId: "cmd_fx1",
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.projectActionTransitioned,
    outcome: "success",
    projectId: "prj_fx1",
    projectActionId: "pac_fx1",
    projectWorkId: "pwk_fx1",
    ownerParticipantId: "ppt_fx1",
    actionRevision: 3,
    fromStatus: "todo",
    toStatus: "doing",
    commandId: "cmd_fx1",
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.projectDecisionCandidate,
    outcome: "unknown",
    projectId: "prj_fx1",
    projectCandidateId: "pca_fx1",
    boundProjectRevision: 1,
    candidateRevision: 1,
    candidateSha256: SHA256_A,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.projectDecisionCommitted,
    outcome: "success",
    projectId: "prj_fx1",
    projectDecisionId: "pdc_fx1",
    decidedByParticipantId: "ppt_fx1",
    boundProjectRevision: 1,
    decisionRevision: 1,
    commandId: "cmd_fx1",
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.projectDecisionRejected,
    outcome: "rejected",
    projectId: "prj_fx1",
    projectCandidateId: "pca_fx1",
    candidateRevision: 2,
    candidateSha256: SHA256_A,
    commandId: "cmd_fx1",
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.projectContributionCandidate,
    outcome: "unknown",
    projectId: "prj_fx1",
    projectCandidateId: "pca_fx1",
    candidateRevision: 1,
    candidateSha256: SHA256_A,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.projectContributionCommitted,
    outcome: "success",
    projectId: "prj_fx1",
    projectContributionId: "pct_fx1",
    participantId: "ppt_fx1",
    contributionRevision: 1,
    evidenceStatus: "reported",
    evidenceCount: 0,
    commandId: "cmd_fx1",
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.projectContributionRejected,
    outcome: "rejected",
    projectId: "prj_fx1",
    projectCandidateId: "pca_fx1",
    candidateRevision: 2,
    candidateSha256: SHA256_A,
    commandId: "cmd_fx1",
  },
  // Workflow
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.workflowStartRequested,
    outcome: "unknown",
    ...wfd,
    workflowDefinitionId: "wfd_fx1",
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.workflowStartStarted,
    outcome: "unknown",
    ...wfd,
    workflowDefinitionId: "wfd_fx1",
    runMappingRef: "wrmap_fx1",
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.workflowStartFailed,
    outcome: "failure",
    ...wfd,
    workflowDefinitionId: "wfd_fx1",
    error: { code: "workflow.unavailable", type: "WorkflowError" },
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.workflowStepStarted,
    outcome: "unknown",
    ...wfd,
    stepKey: "plan.generate",
    stepAttempt: 1,
    replay: false,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.workflowStepCompleted,
    outcome: "success",
    ...wfd,
    stepKey: "plan.generate",
    stepAttempt: 1,
    replay: false,
    outputRefs: [planRef],
    durationMs: 3200,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.workflowStepFailed,
    outcome: "failure",
    ...wfd,
    stepKey: "execute.step",
    stepAttempt: 2,
    replay: true,
    error: err,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.workflowStepReplayed,
    outcome: "success",
    ...wfd,
    stepKey: "plan.publish",
    stepAttempt: 1,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.workflowMemoryNodeStarted,
    outcome: "unknown",
    ...run,
    workflowNodeRunId: "wnr_memoryquery1",
    definitionNodeId: "memory-planning.query",
    nodeType: "memory.query",
    publicSummary: "正在查询Memory",
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.workflowMemoryNodeCompleted,
    outcome: "success",
    ...run,
    workflowNodeRunId: "wnr_memoryquery1",
    definitionNodeId: "memory-planning.query",
    nodeType: "memory.query",
    outcomeCode: "success",
    publicSummary: "Memory查询完成，冻结2条快照",
    durationMs: 12,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.workflowMemoryNodeFailed,
    outcome: "failure",
    ...run,
    workflowNodeRunId: "wnr_memorywrite1",
    definitionNodeId: "memory-planning.write",
    nodeType: "memory.write",
    outcomeCode: "failed",
    publicSummary: "Memory写入失败",
    error: { code: "memory.write_failed", type: "MemoryWriteError" },
    durationMs: 20,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.workflowMemoryNodeOutcomeUnknown,
    outcome: "unknown",
    ...run,
    workflowNodeRunId: "wnr_memorywrite2",
    definitionNodeId: "memory-planning.write",
    nodeType: "memory.write",
    outcomeCode: "outcome_unknown",
    publicSummary: "Memory写入结果未知，等待对账",
    error: { code: "memory.outcome_unknown", type: "MemoryWriteError" },
    durationMs: 20,
  },
  // Plan候选
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.planCandidateReceived,
    outcome: "unknown",
    ...run,
    candidateSha256: SHA256_A,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.planCandidateRejected,
    outcome: "rejected",
    ...run,
    candidateSha256: SHA256_A,
    error: { code: "plan.schema_invalid", type: "ValidationError" },
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.planCandidatePublished,
    outcome: "success",
    ...run,
    planRef,
  },
  // Note候选只保留Hash与对象身份上下文，不含标题、正文或标签。
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.noteCandidateReceived,
    outcome: "unknown",
    ...run,
    candidateSha256: SHA256_B,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.noteCandidateRejected,
    outcome: "rejected",
    ...run,
    candidateSha256: SHA256_B,
    error: { code: "note.candidate_rejected", type: "NoteCandidateError" },
  },
  // Approval与Decision
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.approvalCreated,
    outcome: "success",
    ...run,
    approvalRequestId: "apr_fx1",
    planRef,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.decisionCommitted,
    outcome: "success",
    ...run,
    commandId: "cmd_fx1",
    decisionKind: "approve",
    decisionRef,
    planRef,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.decisionRejected,
    outcome: "rejected",
    ...run,
    commandId: "cmd_fx1",
    decisionKind: "approve",
    error: { code: "decision_hash_mismatch", type: "ConflictError" },
    planRef,
  },
  // Hook
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.workflowHookWaiting,
    outcome: "unknown",
    ...wfd,
    waitReason: "plan_approval",
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.workflowHookResumeDispatched,
    outcome: "success",
    ...wfd,
    resumeAttempt: 1,
    decisionRef,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.workflowHookResumed,
    outcome: "success",
    ...wfd,
    resumeAttempt: 1,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.workflowHookResumeFailed,
    outcome: "failure",
    ...wfd,
    resumeAttempt: 1,
    error: { code: "workflow.hook_expired", type: "WorkflowError" },
  },
  // Provider
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.providerRequestStarted,
    outcome: "unknown",
    ...model,
    ...providerShared,
    inputManifestSha256: SHA256_B,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.providerRequestCompleted,
    outcome: "success",
    ...model,
    ...providerShared,
    httpStatus: 200,
    providerRequestId: "req-0a1b2c",
    tokenUsage: { promptTokens: 100, completionTokens: 50, totalTokens: 150 },
    inputManifestSha256: SHA256_B,
    durationMs: 1234,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.providerRequestFailed,
    outcome: "failure",
    ...model,
    ...providerShared,
    error: { code: "provider.timeout", type: "TimeoutError" },
    durationMs: 30000,
    httpStatus: 504,
    providerRequestId: "req-0a1b2c",
    inputManifestSha256: SHA256_B,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.providerRequestFailed,
    outcome: "failure",
    ...model,
    ...providerShared,
    error: { code: "provider.pre_request.auth", type: "AuthError" },
    durationMs: 12,
  },
  // Pi Coding Agent Operation / Session / Turn / Tool安全事件
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.piOperationAccepted,
    outcome: "unknown",
    ...piOperation,
    requestSha256: SHA256_A,
    workspaceRootId: "root_chat",
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.piOperationStarted,
    outcome: "unknown",
    ...piOperation,
    requestSha256: SHA256_A,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.piOperationCompleted,
    outcome: "success",
    ...piOperation,
    requestSha256: SHA256_A,
    resultSha256: SHA256_B,
    durationMs: 5000,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.piOperationFailed,
    outcome: "failure",
    ...piOperation,
    requestSha256: SHA256_A,
    error: { code: "executor.session_failed", type: "PiExecutorError" },
    durationMs: 5000,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.piOperationOutcomeUnknown,
    outcome: "unknown",
    ...piOperation,
    requestSha256: SHA256_A,
    error: { code: "executor.operation_interrupted", type: "PiExecutorError" },
    durationMs: 5000,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.piSessionStarted,
    outcome: "unknown",
    ...piRuntimeSession,
    enabledTools: ["read", "edit", "write", "bash"],
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.piSessionSettled,
    outcome: "success",
    ...piRuntimeSession,
    turnCount: 3,
    providerRequestCount: 3,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.piTurnStarted,
    outcome: "unknown",
    ...piRuntimeSession,
    turnIndex: 0,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.piTurnCompleted,
    outcome: "success",
    ...piRuntimeSession,
    turnIndex: 0,
    durationMs: 800,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.piMessageCompleted,
    outcome: "success",
    ...piRuntimeSession,
    messageIndex: 1,
    messageRole: "assistant",
    contentSha256: SHA256_C,
    visibleText: "正在执行已批准步骤",
    visibleTextTruncated: false,
    providerStopReason: "toolUse",
    tokenUsage: { promptTokens: 100, completionTokens: 50, totalTokens: 150 },
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.piToolIntentPersisted,
    outcome: "unknown",
    ...piRuntimeSession,
    turnIndex: 0,
    toolCallId: "call_1",
    toolName: "edit",
    inputSha256: SHA256_A,
    inputDisplay: '{"path":"src/index.ts"}',
    inputDisplayTruncated: false,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.piToolBlocked,
    outcome: "rejected",
    ...piRuntimeSession,
    turnIndex: 0,
    toolCallId: "call_blocked",
    toolName: "read",
    inputSha256: SHA256_A,
    inputDisplay: '{"path":"../secret"}',
    inputDisplayTruncated: false,
    error: { code: "executor.tool_path_outside_workspace", type: "PiToolError" },
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.piToolCompleted,
    outcome: "success",
    ...piRuntimeSession,
    turnIndex: 0,
    toolCallId: "call_1",
    toolName: "edit",
    resultSha256: SHA256_B,
    resultDisplay: "Updated src/index.ts",
    resultDisplayTruncated: false,
    durationMs: 20,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.piToolFailed,
    outcome: "failure",
    ...piRuntimeSession,
    turnIndex: 0,
    toolCallId: "call_2",
    toolName: "bash",
    resultSha256: SHA256_B,
    resultDisplay: "command exited 1",
    resultDisplayTruncated: false,
    error: { code: "executor.tool_failed", type: "PiToolError" },
    durationMs: 20,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.piToolOutcomeUnknown,
    outcome: "unknown",
    ...piRuntimeSession,
    turnIndex: 0,
    toolCallId: "call_3",
    toolName: "bash",
    inputSha256: SHA256_A,
    inputDisplay: '{"command":"pnpm test"}',
    inputDisplayTruncated: false,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.piCompactionStarted,
    outcome: "unknown",
    ...piRuntimeSession,
    reason: "threshold",
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.piCompactionCompleted,
    outcome: "success",
    ...piRuntimeSession,
    reason: "threshold",
    aborted: false,
  },
  // pi节点
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.piNodeStarted,
    outcome: "unknown",
    ...model,
    nodeKind: "note_capture",
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.piNodeCompleted,
    outcome: "success",
    ...model,
    nodeKind: "executor",
    candidateRef,
    durationMs: 5400,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.piNodeFailed,
    outcome: "failure",
    ...model,
    nodeKind: "planner",
    error: err,
  },
  // 执行验证
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.executionValidated,
    outcome: "success",
    ...run,
    candidateRef,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.executionRejected,
    outcome: "rejected",
    ...run,
    candidateRef,
    error: { code: "execution.criteria_unmet", type: "ValidationError" },
  },
  // Product Commit
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.productCommitStarted,
    outcome: "unknown",
    ...run,
    outputRefs: [messageRef],
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.productCommitCommitted,
    outcome: "success",
    ...run,
    outputRefs: [messageRef],
    durationMs: 9,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.productCommitFailed,
    outcome: "failure",
    ...run,
    error: { code: "store.cas_conflict", type: "StoreError" },
  },
  // 调试生命周期
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.serviceDebugStarted,
    outcome: "unknown",
    role: "api",
    port: 43111,
  },
  {
    ...base,
    eventName: TRACE_EVENT_NAMES.serviceDebugStopped,
    outcome: "success",
    role: "workflow",
    port: 43112,
  },
];
