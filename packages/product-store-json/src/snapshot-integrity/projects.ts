...
export function assertProjects(snapshot: ProductSnapshot, fail: Fail): void {
  ...
  const activeProviderBindings = new Set<string>();
  const providerProjectBindings = new Set<string>();
  for (const binding of Object.values(entities.projectProviderBindings)) {
    ...
    const reconciledLegacy =
      binding.reconciledWorkspaceBindingId === undefined
        ? undefined
        : entities.projectWorkspaceBindings[binding.reconciledWorkspaceBindingId];
    const matchingLegacy = Object.values(entities.projectWorkspaceBindings).find(
      (candidate) =>
        candidate.providerKind === binding.providerKind &&
        candidate.planeProjectId === binding.externalProjectId,
    );
    if (project?.ownerPrincipalId !== binding.ownerPrincipalId) {
      fail(`projectProviderBinding ${binding.projectProviderBindingId} Project/Owner无效`);
    }
    if (
      (matchingLegacy !== undefined &&
        (reconciledLegacy === undefined ||
          reconciledLegacy.projectWorkspaceBindingId !==
            matchingLegacy.projectWorkspaceBindingId)) ||
      (reconciledLegacy !== undefined &&
        (reconciledLegacy.ownerPrincipalId !== binding.ownerPrincipalId ||
          reconciledLegacy.providerKind !== binding.providerKind ||
          reconciledLegacy.planeProjectId !== binding.externalProjectId ||
          reconciledLegacy.planeProjectIdentifier !== binding.externalProjectIdentifier))
    ) {
      fail(`projectProviderBinding ${binding.projectProviderBindingId} 旧Binding对账无效`);
    }
    const providerProjectKey = `${binding.providerKind}\0${binding.externalProjectId}`;
    if (providerProjectBindings.has(providerProjectKey)) {
      fail(`projectProviderBinding ${binding.projectProviderBindingId} 外部Project重复`);
    }
    providerProjectBindings.add(providerProjectKey);
    const key = `${binding.projectId}\0${binding.providerKind}`;
    if (binding.status === "active") {
      if (activeProviderBindings.has(key)) {
        fail(`project ${binding.projectId} 存在多个活动${binding.providerKind} Binding`);
      }
      activeProviderBindings.add(key);
    }
  }
  const projectionKeys = new Set<string>();
  const providerObjectKeys = new Set<string>();
  const externalProjectionKeys = new Set<string>();
  for (const projection of Object.values(entities.projectProviderProjections)) {
    const binding = entities.projectProviderBindings[projection.bindingId];
    const object =
      projection.objectType === "work"
        ? entities.projectWorks[projection.objectId]
        : projection.objectType === "practice_revision"
          ? entities.projectPracticeRevisions[projection.objectId]
          : projection.objectType === "context_page"
            ? entities.projectContextMaps[projection.objectId]
            : projection.objectId === projection.projectId
              ? entities.projects[projection.projectId]
              : undefined;
    const key = `${projection.bindingId}\0${projection.objectType}\0${projection.objectId}`;
    const providerObjectKey = `${projection.bindingId}\0${projection.providerObjectType}\0${projection.providerObjectId}`;
    const externalProjectionKey =
      projection.externalKey === undefined
        ? undefined
        : `${projection.bindingId}\0${projection.externalKey}`;
    const providerTypeInvalid =
      (projection.objectType === "work") !== (projection.providerObjectType === "work_item");
    const externalKeyInvalid =
      projection.objectType === "work"
        ? projection.externalKey !== entities.projectWorks[projection.objectId]?.workKey
        : projection.externalKey !== undefined;
    if (
      binding?.projectId !== projection.projectId ||
      object?.projectId !== projection.projectId ||
      providerTypeInvalid ||
      externalKeyInvalid ||
      (projection.syncStatus === "healthy" && object.revision !== projection.chatObjectRevision)
    ) {
      fail(`projectProviderProjection ${projection.projectProviderProjectionId} 绑定无效`);
    }
    if (projectionKeys.has(key)) {
      fail(`projectProviderProjection ${projection.projectProviderProjectionId} 映射重复`);
    }
    projectionKeys.add(key);
    if (providerObjectKeys.has(providerObjectKey)) {
      fail(`projectProviderProjection ${projection.projectProviderProjectionId} Provider对象重复`);
    }
    providerObjectKeys.add(providerObjectKey);
    if (externalProjectionKey !== undefined) {
      if (externalProjectionKeys.has(externalProjectionKey)) {
        fail(`projectProviderProjection ${projection.projectProviderProjectionId} externalKey重复`);
      }
      externalProjectionKeys.add(externalProjectionKey);
    }
  }
