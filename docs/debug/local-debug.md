...
-> chat-debug:prepare-memory（再次安全preclean，准备固定提交源码缓存）
-> Chat：Memory（memmy）（18960，Inspector 43122）
-> chat-debug:wait-memory 等待 /api/v1/health
-> Chat：Workflow 运行时（43112，Inspector 43121）
-> chat-debug:wait-workflow 等待 /healthz
-> Chat：API（43111，Inspector 43120；同时确认Workflow与memmy已就绪）
...
- 进程登记：被调试进程通过`node --import scripts/debug/register-process.mjs`
  把`role/pid/port`写入`.data/debug/pids.json`；Memory登记的是包装进程，包装进程收到停止信号后会安全转发给真实memmy子进程；Web由`start-web.mjs`登记进程组。
...
- 单独启动 **“Chat：Workflow 运行时”** 前，必须先启动并确认Memory就绪；该配置只等待
  memmy健康检查，不会自行启动或清理Memory。主Compound会自动安排此顺序。
...
- Workflow与Memory均为真实本地运行时；API只有在两者的健康检查均通过后才启动。
