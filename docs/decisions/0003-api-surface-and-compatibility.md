...
结果。106条路由逐条从`c.req`来源、Query parser/allowed keys、`c.json`/response helper和真实Application
operation数据流生成path/query/body、成功响应与Problem/Recovery合同；输入Schema位于return表达式也不会
被误判为响应，路由引用但未从public barrel导出的Contracts Schema也按真实声明闭包冻结，无法解析时
失败关闭。Package export冻结key、target、conditions、bin与公共入口符号签名。CI还把当前生成
...
代价是合法兼容新增也要审查、记录用途与回滚并更新baseline。
