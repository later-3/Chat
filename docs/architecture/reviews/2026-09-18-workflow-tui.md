...
按[贡献流程](../../development/agent-contribution.md)、[模块合同](../chat-module-contracts.md)、[Session 架构](../chat-session-architecture.md)和[Workflow 框架](../chat-workflow-framework.md)，终端定义为 Backend 的 HTTP 消费者。运行事实仍由 Workflow Runtime 与 Pi SessionManager 拥有，模型、配置、资源和装配继续复用服务端入口。实现不修改 Pi 或 NanoClaw，不复制 Agent Loop。
