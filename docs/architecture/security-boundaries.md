...
| ⑤ | Executor 能力边界：Direct 默认继承真实 `pi_cli_default`；Tool以qualified Capability + 来源/实现Hash冻结；只读/自定义限制只能来自用户选择的 Agent Version、Workflow 或会话临时配置；Workspace 仍须显式授权 | `packages/pi-runtime/src/capability-catalog.ts`、`packages/pi-runtime/src/direct-agent-executor.ts`、`packages/application/src/prompt-assembly-use-cases.ts`、`apps/pi-executor` | Extension覆盖built-in或加载diagnostic失败关闭；Runtime Manifest、Workspace、实现或资源正文Hash漂移在 Provider 前失败关闭 |
| ⑤-A | 高影响Tool动作边界：`local_write/shell/external_write`在handler前必须拥有绑定Intent revision、Capability descriptor、参数Hash和scope的Product Decision | `packages/application/src/tool-execution-use-cases.ts`、`packages/pi-runtime/src/tool-execution-gate.ts`、`packages/dsh-lifeos-bridge` | reject为未执行；claim再次复核Run/Attempt；Journal Result先于Product Result；响应未知只重放Result Receipt，不重放Tool；无法证明时收敛`outcome_unknown` |
