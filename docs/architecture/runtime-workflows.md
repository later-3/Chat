...
7. `MemoryWriteWorkflow`：直接Memory Write Command或已批准Memory Agent候选产生的一次外部写入或一次只读对账；Memory Planning、Memory Direct和Memory Agent Direct只复用其Application状态机，不启动竞争的Workflow。
8. 历史`MemoryImportWorkflow`：只保留旧事实兼容。
9. `ProjectIntakeWorkflow`：一次真实资源建项理解、候选审核和确认。
10. `ProjectAdvancementWorkflow`：现有Project的一次Stage/Milestone/负责人Update理解、候选修订和确认。
...
### 5.1 MemoryAgentDirectWorkflow（`direct@3 / memory-agent-direct.v1`）
...
5. 独立Memory Direct三节点Workflow、Memory-aware Direct Input Manifest、Provider前不可信Context注入、组合Token预算门和候选后Write政策；当前确定性纵向已完成。默认-off DSH管理页真实Chromium门已通过，但显式启用Provider的完整`direct@3` DSH纵向尚未运行。
...
8. `direct@3 / memory-agent-direct.v1`：Retrieval/Write Agent的耐久Operation栅栏、只读结果选择、证据绑定Candidate、人工Decision与批准后复用统一Memory Write状态机；百炼`qwen3.7-plus`真实Memory Agent门3/3通过（Retrieval选择Provider原始index 1，Write生成1个候选）。
9. DSH全局Memory管理页和严格`/lifeos/memory/*`同源代理；确定性Controller/Host合同测试覆盖候选决定的CAS/原样重试、Session Preview/Import冻结Hash及逐条路由校验。默认-off真实Chromium门2/2通过：API空Registry经同源`/lifeos/memory/providers`返回空Provider，Settings→Memory三页签在桌面与390px均无页面横向溢出。
...
1. 显式启用Memory Provider的完整`direct@3` DSH纵向：验证Memory Agent Direct、管理页和用户可见采用详情；尚未运行，不能由默认-off Chromium 2/2、独立真实Provider门或确定性Controller/Host合同测试替代。
2. Chat公开SSE Cursor Runtime Journal。
3. Project Context进入Planning Workflow的节点；PS1已实现Project、初始Stage、Work/Action和资源观察，但尚未注入任务规划。
4. 用户规则选择与规划注入节点。
5. 生产多实例Store、正式身份、Worker生产接管和后端部署拓扑。
6. 外部副作用Tool与通用Workflow编辑器。
