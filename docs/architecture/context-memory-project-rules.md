...
| Workflow Checkpoint | Vercel Workflow Store | 仅保存后端私有映射和产品引用 | 暴露给浏览器或当成产品终态 |
...
|---|---|---|---|---|
| memmy-agent | `211d521b310fc23c63dd3d9ca848941173981c5e`，1.0.4 | 本地 HTTP 服务；`memory/search`、`memory/add`；`adapterId + requestId + requestHash` 幂等；标签和 namespace | 只通过后端 Adapter 调用；查询结果转成 Chat 快照；用户 namespace 由服务端配置映射 | 浏览器直连、自动导出全部对话、把 injectedContext 不经验证直接塞进 Prompt |
...
3. `MemoryResultSnapshot`：外部命中的规范化、不可变快照；保存 externalObjectId、外部版本、正文、Hash、分数、标签、时间和 backendId。
...
interface MemoryBackendPort {
  ...
  import(input: MemoryImportInput): Promise<MemoryImportOutput>;
...
3. `query` 只读，可有限重试；`import` 是外部副作用，发送后失联必须进入 `outcome_unknown`，先对账再决定是否重发。
...
Message Command
  ...
  → assembleContextRequestStep
  → queryMemoryContextStep（仅用户选择/策略要求时）
  → persistContextPackageStep
...
2. 百炼仍固定真实 `qwen3.7-plus`。本地测试优先复用 `/Users/xulater/.pi/agent/read-chat-provider-key.mjs` 从 pi 配置读取到子进程环境；不打印、不写入 Git，也不再次要求用户手工提供已存在的 Key。
3. Memory 服务固定到独立调试端口；启动前使用现有安全 preclean 机制，只清理自己登记且身份匹配的进程，未知占用只报告不杀。
4. 弱服务器只接收本地构建产物；本阶段不在服务器编译，也不自动部署。

## 11. 失败语义

| 场景 | 产品结果 |
|---|---|
| Memory 查询超时 | 若用户标记 required，则 Run 失败关闭；optional 则记录排除原因并继续 |
| 外部响应合同损坏 | `memory.backend_contract_invalid`，不采用任何命中 |
