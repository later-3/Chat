...
1. 是否同意“9个能力域、74项能力”作为完整Session目标全集。
2. 是否同意R0-R6是不同恢复保证，不能用“保存历史”或一个`resume`笼统替代。
3. 是否同意明确非目标：精确续传第三方Token流、通用Exactly-once、隐藏推理存储、分支回滚外部副作用、多人实时编辑和自动分支合并。

阶段顺序、每个任务的方案、目标、依赖和完成场景见[Session分阶段交付路线](./session-delivery-roadmap.md)。两份文档全部通过后，才进入Phase 0的详细设计与兼容性验证；不会直接创建Schema或写业务实现。
