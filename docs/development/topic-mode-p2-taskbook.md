...
- **`topic-manage` 工具**（`src/tools/builtins/topic-manage/**`，`system:tool/topic_manage`，risk `write`，permission `long-agent:topic`）：身份只取运行上下文的 `longAgentId`（模型参数不能指定 Long Agent 或存储项目），仅 `purpose === "execution"` 可用。操作：`read_graph` / `read_node` / `read_memory` / `read_fulltext`（跨树**只读**，走 `authorizeTopicSession(read)`；会话记忆只存在于 Long Agent 归属会话，普通项目会话明确拒绝）/ `create_topic` / `create_node`（调 `createTopicNodeWithSession`，同一 `requestId` 幂等；父边锚点必须 entry+sequence 成对）/ `add_parent`（调 `addTopicNodeParent`）/ `update_node_status` / `relay`。工具**不实现**来源、锚点、幂等或授权判定，全部委托领域服务；需要当前 revision 的写操作经 `withTopicGraphRevision` 在冲突时重读重试。**`relay`**：写入一条**真实 user message** + 耐久标记 `chat.topic-relay {requestId, relayedByLongAgentId, source:"relay", userEntryId}`，按 `requestId` 幂等，仅限自己名下主题树；**触发该轮执行属于节点会话 API/轮次 Workflow**，不在本工具内。
