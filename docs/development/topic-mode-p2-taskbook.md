...
- **每轮 = 一次 workflow（目标；P2 以队列编排等价落地，取舍见 §4“外层编排”）**：`work` step 在当前节点会话跑普通一轮 agent（连续会话逻辑不变）；`remember` step 用 `transformContext`（`src/workflows/agent-definition.ts`）把上下文**投影为当前轮**（本轮用户 entry + `work` stage 全部条目），前文与旧记忆由它按需读取。`triggerChatWorkflowAgentHandoff` 只作触发（它不传上下文）。**P2 实际交付**：`work` 段直接复用 `executeAcceptedLongAgentTurn`，`remember` 段调用与 `remember` step 共用同一实现的 `runSessionMemoryWriterTurn`，两层都由**队列 worker** 顺序编排——**不是**一次 Workflow SDK invocation，因此暂时没有 Workflow run/step 事件；把这条链映射成 Workflow run 与 `.../turns/{turnId}/events` 的后续工作在 §4“外层编排”记录。
...
- **外层编排与提前分叉窗口（已实现）**：**接受/绑定/队列仍是唯一事实源**；`executeAcceptedLongAgentTurn` 就是那个**可复用的 Long Agent 工作执行段**，而**队列 worker 是外层编排点**：工作段 →（节点且记忆为 on）`runSessionMemoryWriterTurn` → **写整轮完成标记** → 才 `updateTurnStatus(completed)`。因此「work 完成、remember 未完成」**不是** settled 轮次。
  - **取舍（如实记录）**：这条链是**队列编排**，不是任务书目标里的“一轮一次 Workflow SDK invocation”。它换来了复用唯一工作段、避免第二套装配与立即可用，但代价是**没有 Workflow run/step 事件**；将来要在 P3/P4 展示“Workflow 节点执行过程”时，**不能**直接把这条队列链当成既有 Workflow run。后续需要把 `work`/`remember` 映射成 Workflow run 与 step 事件（或为队列编排补一层等价事件），属于明确遗留工作，不阻塞本阶段。
  - **writer 只有一份实现**：`src/workflows/session-memory/writer-run.ts` 是普通函数（非 `"use step"`），被 Workflow 的 `remember` step **与**队列 worker 共用；worker 里对该模块用**惰性 import**（静态引入会把 Workflow 装配图并入 Long Agent runtime 图，导致 Nitro dev Step worker 初始化失败：`init_agent_definition is not a function`）。
  - **锚点门禁**：队列 worker 在**工作段启动之前**幂等写入 `chat.topic-round{status:"running"}`（`ensureTopicRoundRunningMarker`，此时还没有用户条目，因此 `running` 标记允许空 `userEntryId`；同一 `turnId` 也用于完成标记）。检查/追加/flush 全程在**节点会话操作锁内**（锁在 work 之前释放，**不**把 work 包进同一把锁，否则自锁），因此 running 标记与随后 work/relay 的写入落在**同一分支**，不会因并发写入成为同父兄弟条目。接受侧只排队、不写标记，因此“已接受、标记未写即中断”的窗口由 worker 在恢复执行时闭合；`readTopicSettledAnchors` **按轮次**（`roundId === turnId`）抑制该轮自己的 `chat.long_agent_turn` completed，未携带整轮标记的历史轮次仍按旧规则可分叉。关闭记忆时，worker 在工作段后直接写 `completed` 整轮标记，即该轮完成。
