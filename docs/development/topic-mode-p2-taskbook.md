...
- **节点只读 API（§3 表格第 6 行读取部分，已实现）**：`GET /api/long-agents/{longAgentId}/topics`（主题图 + 节点 `readable`）、`GET .../topics/{topicId}`（单主题 + 节点 + 边）、`GET .../topics/{topicId}/nodes/{nodeId}/messages`（节点消息，复用 `readChatSession`）。**会话 id 一律由图解析**，客户端从不提供 sessionId，因此该路由不能被指向任意会话；共享主题判定与共享会话读取判定同时生效（`readChatSession` 现在自带存储归属，见下）。
- **节点轮次的耐久状态决策（未实现，需评审）**：`POST .../nodes/{nodeId}/messages` 目前**不可用**。原因不是接线，而是**状态合同**：`parseLongAgentState` 要求每个 turn 必须归属某个 `dailySessions` 记录或某个 `works` 绑定，而节点会话两者都不是，因此节点轮次无法被现有 turn 记录表达。可选方向（需你裁定，涉及 LA5/LA6 已验收的接受合同与 schema）：
  1. **新增节点会话绑定**（推荐）：state 增加 `nodeSessions: [{longAgentId, sessionId, topicId, nodeId}]` 与 schemaVersion 迁移，`parseLongAgentState` 接受归属于它的 turn（`date` 仍取当日、`timeZone` 取该 Agent）；`ensureProjectLongAgent` 增加显式 `topicNodeSessionId` 入口，**不动** `dailySessions` 与 primary 绑定（我已验证该入口能让节点轮次被接受，但只有配上方案 1 的状态记录才可持久化）。
  2. 把节点轮次表达为 `works` 条目：与“后台工作必须有日常来源”的既有合同冲突（任务书已明确 `startFriendWork` 只接受日常来源），不推荐。
