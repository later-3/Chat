...
- **外层编排与提前分叉窗口（已实现）**：**接受/绑定/队列仍是唯一事实源**；`executeAcceptedLongAgentTurn` 就是那个**可复用的 Long Agent 工作执行段**，而**队列 worker 是外层编排点**：工作段 →（节点且记忆为 on）`runSessionMemoryWriterTurn` → **写整轮完成标记** → 才 `updateTurnStatus(completed)`。因此「work 完成、remember 未完成」**不是** settled 轮次。
  ...
  - **锚点门禁**：节点会话在**接受时**就写入 `chat.topic-round{status:"running"}`（此时还没有用户条目，因此 `running` 标记允许空 `userEntryId`）；`readTopicSettledAnchors` 一旦发现该会话存在整轮标记，就**只认 `completed` 整轮标记**，不再把 `chat.long_agent_turn` 的 completed 当作可分叉轮次。关闭记忆时，worker 在工作段后直接写 `completed` 整轮标记，即该轮完成。
