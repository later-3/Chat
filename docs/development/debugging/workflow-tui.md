...
当前TUI只调用普通Workflow。UI来源、提示行、状态栏、Web/NanoClaw关系以[架构合同](../../modules/tui/chat-workflow-tui.md)为准，完整命令和独立安装见[CLI指南](../../modules/tui/README.md)。本章验证终端入口到同一Backend的真实执行链。
...
开发TUI只启动客户端；先在Web打开业务Project，再通过 `/project` 选择。服务端数据仍由Backend的 `CHAT_HOME` 决定。`CHAT_SERVER_URL` 可显式覆盖。VS Code任务 `Chat: open development TUI` 等价于 `dev:tui`。
...
调试Backend为 `45112`、Web为 `35145`、假模型为 `45401`。TUI不占端口。启动器自动构建 `cli/dist` 和Source Map，最多等待Backend就绪60秒，直接通过API登记 `Debug Lab`，最后以 `--project debug-lab` 进入。首次无需先从Web登记项目。不需要产品登录凭据；运行不依赖NanoClaw。

常用 F5 入口：`Debug TUI` 只启动客户端；`Debug Chat TUI` 启动假模型 + Backend + TUI；`Debug Chat Web + TUI` 额外启动 Vite 与独立 Chrome，适合双端同步断点。完整环境中只想正常使用终端时，`Run TUI` 不挂调试器；之后可单独启动 `Debug TUI` 切换为调试。逐步操作见[环境手册](./environment.md#操作示例后端与-nanoclaw-同时调试)。单独 TUI 配置需要 Backend/模型已运行；启动器不会暗中拉起它们。所有 TUI 配置使用 `integratedTerminal`，不能放在 Debug Console 或把 stdout 接到 pipe/tee。
...
| 层次 | 断点 | 关键观察值 |
|---|---|---|
| 终端启动 | `cli/src/main.ts`：`main` | 目标URL、Project参数与响应错误 |
...
Web通常在空闲可见页面3秒轮询后更新，TUI每2秒重读；长历史分页和流恢复详见CLI指南。浏览器使用独立调试profile，隔离站点存储。`Debug Chat TUI` 本身没有启动Web，验证双端请用包含Web的组合。
...
独立 `dev:tui` / `debug:tui` 的 `/quit` 只断开客户端，显式 `/cancel` 才取消 Run。`debug:start --tui` 的 `/quit` 会结束本次整套调试栈。F5 原有 `Debug Chat...` 快捷组合使用 `stopAll: true`，Stop 会收回整组；两个 `(full environment)` 入口使用 `stopAll: false`，停止 TUI 后其他模块继续运行。`pnpm debug:stop -- tui` 只停止有归属记录的 TUI；若它属于 CLI 整套栈，父启动器也会随之收尾。所有调试模块一起停止用 `pnpm debug:stop`。详情见[停止说明](./stopping.md)。
