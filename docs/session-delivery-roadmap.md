...
4. **树兼容从第一阶段预留，产品分支放第三阶段交付**：早期不锁死线性数据，仍先把最高优先级的保存与恢复做对。
...
8. **本通道语义稳定后再跨通道**：否则Web、Telegram或其他通道会各自形成Session、重试和投递事实源。

### 2.3 考虑过的其他拆法

| 选择 | 优点 | 缺点 | 参考项目关系 | 结论 |
|---|---|---|---|---|
| A. 只先做历史持久化，做到哪再规划到哪 | 最快开始写表和CRUD | 看不到分支、Run、断线、Tool和HITL对早期模型的约束，后续返工概率最高 | 只使用了4个参考项目的一小部分 | 拒绝；正是本次要纠正的问题 |
...
| F. 本路线：完整目标先冻结，再按恢复等级逐层实现 | 依赖清楚、每层可验收、早期不锁死后续 | 审核门更多；活动流、Worker和HITL不会在最早阶段同时出现 | 综合M/P/N/L各自最强部分 | 建议 |
...
2. 浏览器断线续传要到Phase 4，Worker恢复要到Phase 5，完整HITL要到Phase 7；最早版本只承诺R0/R1。
3. 每升级MAF/AG-UI都要重跑关键合同，尤其是History、终态、Snapshot和interrupt/resume。
4. 跨通道交付较晚，但换来的是上位系统复用稳定Session事实，而不是多个通道各自补丁式对账。
...
|---|---|---|---|---|
| P0-01 | 用户批准本路线 | 审核并冻结74项能力、R0-R6恢复层级、对象责任和明确非目标；变更需重新审核 | 防止后续用“第一阶段没做”掩盖总体能力缺口 | 4个参考项目的联合结论 |
...
| P0-03 | P0-01、P0-02 | 将现有D1-D6重审为Phase 1的持久化子设计，补上树兼容、Attempt演进和后续Checkpoint关联约束 | 只在总体目标下选择首个实现，不让子设计反向锁死路线 | MAF、pi、nanobot、LibreChat |
...
| P1-02 | P1-01 | 建立从首版就支持父子关系和类型化内容的Message记录；用户输入在模型前提交，Assistant区分流式临时态、部分、错误和最终态 | 不丢已接纳输入，并为后续分支避免破坏性迁移 | N先保存、P/L消息树 |
...
## 12. Phase 8：跨通道、投递与治理闭环

### 12.1 阶段目标

把完整Session作为本自研Web Chat通道的稳定能力接入OPC-OS Chat上位系统，并补齐跨通道投递、长期领域引用、分享、删除、审计和运维闭环。
...
| P8-02 | P1-01、P8-01 | 完成上位系统Channel Binding、来源身份、能力声明和权限撤销传播 | Web及未来其他通道可授权继续同一Product Session | N、O；参考未完整覆盖 |
...
跨通道会放大重复输入、身份映射、并发、投递失败和权限撤销问题。只有单通道的Session、Run、恢复和工具语义稳定后，上位系统才能复用这些事实，而不是建立另一套会话状态。

### 12.4 阶段完成场景

1. 用户从Web开始任务，经授权后在另一个OPC-OS Chat通道继续同一Session和WorkItem。
...
| 能力域 | 首次建立 | 完整交付 | 说明 |
|---|---|---|---|
| A1-A6 身份与边界 | Phase 0-1 | Phase 8 | 首版固定对象边界，跨通道时补全Binding与撤销 |
| B1-B10 生命周期 | Phase 1 | Phase 8 | 基础创建/列表/打开先做；组织、临时、分享和级联随后补全 |
| C1-C9 消息与分支 | Phase 1 | Phase 3 | 数据从首版树兼容，Phase 3开放完整分支操作 |
...
| H1-H6 跨通道与投递 | Phase 1预留 | Phase 8 | 首版只建立Web Binding，最后完成上位系统接入 |
| I1-I7 治理与质量 | Phase 0-1 | Phase 8 | 版本和测试从第一天存在，运营治理随能力补全 |
