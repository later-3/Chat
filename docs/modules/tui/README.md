...
这是本地 tarball 分发方式，尚未发布 npm 包。Backend 必须同时包含本轮新增的 transcript、fork 和活跃 Run 恢复合同；旧服务需要按[部署指南](../../operations/README.md)升级。安装 CLI 不启动另一个 Backend，不迁移服务端数据。远程连接要求 HTTPS；本机 loopback 可以用 HTTP。

`CHAT_SERVER_URL` 可替代 `--url`；默认 `http://127.0.0.1:43110`。
...
客户端、API 和持久化边界见[接入设计](./chat-workflow-tui.md)，验证命令见[测试指南](../../development/testing.md)。
