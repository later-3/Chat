...
Project、分层配置、Session分区和资源Target的现状见[Chat Project架构设计](../projects/chat-project-framework.md)与[Chat Context与Resource Target模型](../../architecture/chat-context-resource-model.md)。Workflow、Agent、Node和Pi装配合同不因资源归属变化而改变。
...
所有Workflow还必须通过[Session消息检查清单](../sessions/chat-session-architecture.md#9-新workflow检查清单)。
