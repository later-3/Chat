...
| D Web 与管理 | 群管理 API/Tool/设置面板与状态展示 | **已验收（本地范围）**：后端 `/api/long-agents/[id]/conversations*`（创建/列表/详情/配置 CAS/成员/archive/messages/discussions/consult/works/stop；严格拒绝未知字段）；Agent 侧只读 `conversation_manage` Tool（身份来自 `toolContext.longAgentId`，只能看自己是成员的群，propose 只返回待确认建议）；Frontend `lib/friend-conversations.ts`（运行时校验、不发送身份）；**交流区**工作区导航新增“群聊”入口与中央 `LongAgentGroupChatView`（复用公共 `MessageView`、公共历史 + SSE 实时流 + 发送消息 + @一轮 + 状态），设置页 `LongAgentConversationsPanel` 专管成员/策略/预算/归档；群详情向 owner 暴露成员参与 Session（`members[].sessionId`），中央视图提供每位成员“完整历史”入口（复用 `FullHistoryDialog` 走 export 只读展示，不提供往参与 Session 直接发送的入口）与公共根“群聊 · 完整历史”入口（原始记录中成员发言为引用 Entry，可读对话流仍以群公共投影为准）。证据：`test/tools/conversation-manage.test.mjs`、`test/long-agents/conversations.test.mjs`（summary 暴露绑定 sessionId）、`frontend/lib/friend-conversations.test.mjs`、`scripts/group-browser.test.mjs`（真实浏览器） |
