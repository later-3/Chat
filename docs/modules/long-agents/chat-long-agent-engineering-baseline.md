...
本轮证据来自当前工作区源码与文档的静态核查。工作区包含既有未提交实现；下文“已有”表示存在代码与相应测试入口；实际验证范围见[本轮审核记录](../../history/reviews/2026-09-07-agent-development-readiness.md)，不代表已部署。当前配置语法仍以[系统配置](../../configuration/README.md)为准。

不再重复讨论已确认的产品方向：NanoClaw承载长期身份映射、Channel、调度与耐久通信；Chat Backend接管产品配置、Project、Pi执行和原生Chat Session；Web是Backend客户端。独立 Agent、统一 Chat 模型与共享概览继续成立；Friend Session/并发按本文开头的 2026-09-19 修订及生命周期合同执行。
...
| C8 | 并发控制落到Session和实际冲突资源；写入原子化或有冲突保护；等待可恢复且不占住整个Agent | 同 Session 有序、独立 Workflow 子工作并行、共享写冲突、互相追问和重启测试 |
...
CustomEntry不承载真实发言的唯一正文。Pi协议角色与真实发起者身份不能混为一谈，已有子Workflow的委派来源是参考；多方投影需要按[Session架构](../sessions/chat-session-architecture.md)验证消息/工具调用配对、分支、压缩、恢复与导出。新元数据需声明版本、旧格式读取、缺失/未知版本处理及索引重建，不只增加写入函数。
...
2026-09-08补充：[系统生命周期合同](../../architecture/chat-system-lifecycle.md)进入B0/B1的横向前置输入，统一实例归属、启停、就绪与在途收尾。现有开发脚本不管理Nano，不能作为完整Long Agent系统启停的验收。
...
| B3 | 道德经与另一Project切换、隔天继续；Coder能查共享概览；同 Friend 当天跨项目不拆会话，显式 Workflow 子工作按自身合同并行 | B1、B2 | 学习、项目工作、历史、日记和跨入口连续性 |
...
| 消息与Session | 原生发言、CustomEntry关系、压缩/恢复、跨 Channel 续接、按日轮换及项目交接 |
...
实际命令以[测试指南](../../development/testing.md)、[Nano规则](../../../nanoclaw/AGENTS.md)和[Frontend规则](../../../frontend/AGENTS.md)为准。Workflow/公共装配/可达资源修改必须覆盖Builder转换、开发Step bundle、生产与实际Runtime；使用既有pnpm test:dev门禁，不另造相邻链路替代。
