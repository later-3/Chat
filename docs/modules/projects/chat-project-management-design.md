...
状态：实现于当前工作区；部署运行版本由父仓库发布决定。本文是 Project 管理能力的技术合同，[配置说明](../../configuration/README.md#project-管理-skill-与-tool)为使用入口。Skill 正文唯一源码为 [project-management/SKILL.md](../../../src/resources/builtin-skills/project-management/SKILL.md)。
...
名称原样保留中文与空格（去除首尾空白），工作目录使用该名称；占用时追加 ` (2)` 等序号，显示名不变，不覆盖已有目录或符号链接。名称须为安全单级目录名且UTF-8最多240字节。内部ID默认是128位随机标识，无project前缀；可选id只控制稳定身份，不决定显示名或工作目录。运行数据仍按ID隔离。不要求模型翻译中文或拼装路径。托管项目仍是普通 Project，只有保留 ID `daily` 具有默认角色。配置默认继承，不自动安装额外资源或选择模型。
...
| `project_create` | name、description?、id?、requestId | 按名称创建托管目录，ID独立；同请求重试返回 existing |
...
1. 对可信来源 Session + requestId 派生标记 ID，串行写入 `<CHAT_HOME>/runtime/project-operations/<hash>.json`，保存分配 projectId、workspaceName、参数摘要和 pending 状态。单Backend创建锁同时串行分配身份与目录；已有pending标记保留目录名，即使尚未mkdir也不被另一请求抢占。显式ID与已有登记、运行数据或创建标记冲突时失败。
2. 幂等初始化本操作目录、Manifest 和最小配置，拒绝身份冲突与符号链接越界。
3. 经原 openProject 登记，再标记 completed。
4. 同键同参数恢复原项目，同键不同参数报冲突；不通过重试分配新 ID，不自动删除失败目录。

旧schemaVersion 1标记没有workspaceName时，继续使用原projectId目录，并兼容未包含可选id的旧参数摘要；不迁移已存在项目。新版标记仍为版本1的兼容字段扩展，已完成请求始终通过Registry解析原项目。
