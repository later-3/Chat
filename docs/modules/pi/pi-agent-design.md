...
Chat使用`CustomEntry`记录Workflow、Stage、Agent身份、配置快照、控制状态和原生消息引用。内部Agent只要产生了真实话语，仍必须保存为原生`message`；不能把输出正文只放进CustomEntry。完整约束见[Chat Session架构](../sessions/chat-session-architecture.md)。
