...
>
> **术语速查**:文中高频术语(耐久、编排、幂等、checkpoint、重放、乐观锁等)的解释和例子,见[架构概览.md §0 读前必懂](file:///Users/xulater/Code/Chat/docs/user-note/basic/架构概览.md),读到卡壳可先回那里对一下。
...
整个流程是**耐久**的——每个步骤都持久化到磁盘,进程崩溃可恢复。这类似游戏的存档点机制,但粒度更细(每个 step 一个 checkpoint)。

### 面向谁?

本文面向会一点 C++、刚开始接触 TypeScript / React / Web 开发的开发者。第 1.1-1.3 节提供概念映射和语法速查,帮助你跨越语言和生态的鸿沟:1.1 用 C++ / 通用编程概念映射系统架构,1.2 讲进程间通信,1.3 提供 TypeScript 语法对照 C++ 的速查表.
...
### 1.1 用 C++ / 通用编程概念映射系统架构

如果你会 C++,以下映射帮助你快速理解本系统的架构概念(用你已有的知识接住新概念):

| 本系统概念 | C++ / 通用编程类比 | 说明 |
|-----------|------------------|------|
| **Product Store** (JSON 文件) | 数据库 / 数据文件 | 权威状态存储,只读视图给外部,写入走原子事务 |
| **原子事务提交** | 数据库事务 | `product.transaction.started → committed`,要么全成功要么全回滚 |
| **Outbox** | 事务发件箱 | 延迟派发队列,后端写 Outbox 项,dispatcher 异步推给 Workflow Runtime |
| **Workflow Runtime** | 后台常驻进程 | 后台长驻进程,循环接收并执行工作流任务 |
| **Hook 等待** | 程序暂停等输入(状态存盘) | 工作流暂停在 Hook 上,等外部事件(用户决定)唤醒 |
| **Hook 恢复** | 外部唤醒继续 | 用户决定写入后,Outbox 派发 resume,唤醒 Hook |
| **Checkpoint** | 游戏存档点 | 每个 step 后保存执行状态到 `.data/workflow/`,崩溃后从最近 checkpoint 恢复 |
| **重放(Replay)** | 读档继续 | 恢复时重放已完成的 step(返回缓存结果),跳到未完成的 step 继续 |
| **Trace JSONL** | 追加写的事件日志 | 追加写 JSON 文件,每行一个事件,按时间排序 |
| **CAS 乐观锁** | 乐观锁版本号 | Run 的 `revision` 字段,提交时带 `expectedRevision`,不匹配返回 409 |
| **SWC 打包** | `gcc -c` + `ld` 编译链接 | TypeScript 源码编译打包成 `.mjs` bundle,运行时动态 `import()` 加载 |
| **tsx 直接执行** | 解释执行(保留源码位置) | 非 step 模块的 `.ts` 文件由 tsx 直接解释执行,保留源码映射(可调试) |
| **async/await** | 协程 / future | `async function` 返回 Promise,`await` 阻塞等待完成 |
| **Promise** | `std::future` | 异步操作的句柄,完成后可 await 拿结果 |
| **Zod 校验** | `static_assert`(运行时版) | 运行时类型校验,HTTP 响应不符合 schema 直接报错(合同损坏) |
| **Vercel Workflow SDK** | 编译期代码生成 | `"use workflow"` 函数体被 SDK 编译转换(注入 step.run 包装),类似编译器对带注解函数做代码改写 |
...
| 通信路径 | 方式 | 直观理解 |
|---------|------|---------|
| 浏览器 → 后端 API | HTTP REST (POST/GET) | 普通的网页 HTTP 请求 |
| 后端 API → Workflow Runtime | Outbox 派发 (HTTP POST) | 事务发件箱派发任务 |
| Workflow Runtime → 后端 API | HTTP internal API (POST) | 调后端的内部接口 |
| Workflow Runtime → 百炼 LLM | HTTPS (DashScope API) | 远程调用大模型 API |
| 前端 ← 后端(状态更新) | HTTP 轮询(1.5s) | 前端每 1.5 秒问一次"好了没" |

> **为什么用 HTTP 而不是函数调用?** 三个进程独立运行,各自有自己的内存空间和生命周期,不能像函数调用那样直接调。HTTP 是进程间通信的通用方式:跨进程时数据必须序列化成 JSON 文本传过去,对方再解析回来。就像两个独立程序之间只能靠发消息交流,不能共享变量.
...
> **Product Store 原子提交**
>
> Product Store 是一个 JSON 文件(`.data/product/chat-product-store.v1.json`),但写入不是直接覆盖。原子提交机制类似数据库事务:
>
...
> 3. **幂等保护**:`commandId` 记录在 `commandReceipts` 中。同一个 `commandId` 重发不会创建第二条消息(像请求带唯一编号,重发不重复执行)。`sourceMessageSha256` 防止内容篡改。
>
> **Outbox 派发**(事务发件箱模式):
>
> 步骤 6 写入的 Outbox 项包含 `workflow_start` 任务。后端有一个 dispatcher(后台派发器)循环扫描 Outbox,将未派发的任务 POST 给 Workflow Runtime (port 43112)。Workflow Runtime 接收后启动 `planningExecutionWorkflow`。
>
> **为什么不让 HTTP 请求直接调用 Workflow Runtime?** 因为 HTTP 请求必须在 Product Store 提交后才返回 201,而 Workflow 执行可能需要几十秒。Outbox 解耦了"提交事实"和"执行工作流":先把数据和待办任务一起落库,立刻返回成功;后台派发器再异步把任务发给工作流。用户不用等几十秒模型调用.
#### HTTP 响应
...
> **为什么 `"use workflow"` / `"use step"` 要打包?**
>
> 这类似 C++ 编译器对带注解函数做代码改写。`"use workflow"` 是编译期指令(类似给函数加 `__attribute__` 注解),告诉 SDK 这个函数不是普通函数,而是耐久工作流定义。SDK 必须转换它:
>
> 1. **注入 checkpoint 逻辑**:在每个 `await xxxStep()` 调用前后,SDK 注入状态保存/恢复代码(类似编译器在函数边界自动插入栈帧管理代码)。你的源码 `await beginPlanningContextStep(...)` 被转换成 `step.run("beginPlanningContextStep", async () => { ... })`,SDK 在 `step.run` 内部做 checkpoint。
>
> 2. **序列化加载**:转换后的函数体打包成纯 JS 字符串(bundle),Runtime 通过动态 `import()` 加载(类似程序运行时动态加载一个编译好的模块)。这保证了生产环境和本地环境执行的是同一份 bundle。
>
> 3. **sourcemap 缺失**:SWC 转换改变了函数结构(注入了 `step.run` 包装),即使生成 sourcemap,行映射也不精确。就像 `gcc -O2` 优化后,源码行和机器指令的对应关系也会乱,调试器难精确断点。
>
> **调试策略**:非 step 模块(如 `api-client.ts`)不参与打包,由 tsx 解释执行(像脚本解释器,保留源码位置),保留源码映射,断点可命中.
...
> **Hook 机制详解**
>
> Hook 就是工作流的"耐久暂停点":像游戏暂停在"按 A 确认 / 按 B 取消"的对话框,状态存了盘,随时能继续。工作流执行到 `planDecisionHook()` 时:
>
> 1. **注册等待**:Runtime 在 `.data/workflow/` 写入一条 Hook 等待记录,包含 `productRunId` 和 `approvalRequestId`(把"我在等什么"存成文件)。
> 2. **暂停执行**:工作流函数挂起,不消耗 CPU。Runtime 可以处理其他工作流任务(暂停的空当去干别的)。
> 3. **外部唤醒**:用户提交 approve 后,后端写 Outbox resume 任务,dispatcher 推给 Runtime。Runtime 查到对应的 Hook 记录,恢复工作流执行(有人按了确认键,继续)。
> 4. **耐久性**:即使 Runtime 进程崩溃重启,Hook 等待记录仍在磁盘上。Runtime 重启后扫描 `.data/workflow/`,恢复所有未完成的 Hook 等待(存档还在,接着等)。
> 5. **超时保护**:Hook 有 `expiresAt`(24小时),超时后工作流自动恢复,走 `expireApproval` 路径(等太久没人决定,超时失败)。
