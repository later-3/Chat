...
> 2026-08-05重入说明：用户已冻结TypeScript全栈与`pi-agent-core`目标方向。本文已批准的产品问题、
> 状态所有权和产品不变量继续作为重构输入；所有MAF/AG-UI技术落点、运行责任和交付依赖已进入修订，
> 不再代表当前目标实现。修订必须先完成[RP-01](./pi-native-replatform-plan.md)、Memory重审和用户逐项审核。
