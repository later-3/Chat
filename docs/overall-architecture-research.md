...
> 2026-08-05重入说明：本文保留为旧技术基线的研究证据。用户已经批准TypeScript全栈与
> `pi-agent-core`目标方向；MAF/AG-UI映射只可作为现有行为预言机，不能继续外推为目标架构事实。
> 新一轮技术研究从[RP-01](./pi-native-replatform-plan.md)开始，完成后再修订本文的目标技术映射。
