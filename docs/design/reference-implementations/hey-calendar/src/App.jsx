...
function Avatar() {
  return <img className="avatar" src="/assets/profile-avatar.png" alt="Maya Chen" />;
}
...
function SometimeStrip({ state, setState, onAdd, compact = false }) {
  return (
    <section className={`sometime-strip ${compact ? "is-compact" : ""}`} aria-label="Sometime this week">
      <strong>Sometime this week</strong>
      <div>{state.tasks.map((task) => <button type="button" key={task.id} className={task.completed ? "is-complete" : ""} aria-pressed={task.completed} onClick={() => setState((current) => toggleTask(current, task.id))}><span>{task.completed ? <Check size={14} weight="bold" /> : null}</span>{task.title}</button>)}<button type="button" className="add-sometime" aria-label="Add Sometime this week task" onClick={onAdd}><Plus size={16} /></button></div>
    </section>
  );
}
...
function DayView({ state, setState, route, navigate, onOpenEvent, onNewAt, onEmail, onJournal, onHabits, onAddTask, onDecorate }) {
  ...
  return (
    <main className="calendar-main day-main">
      ...
      <SometimeStrip state={state} setState={setState} onAdd={onAddTask} />
...
function WeekView({ state, setState, route, navigate, onOpenEvent, onAddTask }) {
  ...
  return (
    <main className="calendar-main week-main">
      ...
      <div className="week-stack">
        ...
        <SometimeStrip state={state} setState={setState} onAdd={onAddTask} compact />
        <WeekSection state={state} date={addDays(currentStart, 7)} currentDate={route.date} onSelectDate={(date) => navigate({ view: "day", date })} onOpenEvent={onOpenEvent} />
      </div>
    </main>
  );
...
function SearchDialog({ state, query, setQuery, onClose, onOpenEvent, onGoDate, onGoSometime }) {
  ...
  return (
    <div className="modal-layer search-layer" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section className="search-dialog" role="dialog" aria-modal="true" aria-label="Search calendar">
        <header><MagnifyingGlass size={22} /><input autoFocus aria-label="Search events, Sometime tasks, and Journal" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search events, Sometime, and Journal…" /><button type="button" aria-label="Close search" onClick={onClose}><X size={19} /></button></header>
        <div className="search-results">{query && results.length === 0 && <p>No results in visible calendars.</p>}{results.map((result) => <button type="button" key={`${result.type}-${result.id}`} onClick={() => { if (result.type === "Event") onOpenEvent(result.id); else if (result.type === "Sometime") onGoSometime(); else if (result.date) onGoDate(result.date); }}><span>{result.type}</span><strong>{result.title}</strong>{result.date && <small>{formatShortDate(result.date)}</small>}</button>)}</div>
...
function AddTaskDialog({ state, setState, onClose, announce }) {
  const [title, setTitle] = useState("");
  const add = () => { if (!title.trim()) return; setState((current) => addTask(current, title)); announce("Added to Sometime this week"); onClose(); };
  return <div className="modal-layer" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="task-dialog" role="dialog" aria-modal="true" aria-labelledby="task-title"><header><h2 id="task-title">Sometime this week</h2><button type="button" aria-label="Close Sometime task" onClick={onClose}><X size={18} /></button></header><input autoFocus value={title} onChange={(event) => setTitle(event.target.value)} onKeyDown={(event) => event.key === "Enter" && add()} placeholder="What needs doing this week?" /><footer><button type="button" onClick={onClose}>Cancel</button><button type="button" disabled={!title.trim()} title={!title.trim() ? "Write a task first" : "Add task"} onClick={add}>Add</button></footer></section></div>;
}
...
export function App() {
  ...
  }, [route, state.draftEvent, searchOpen, journalOpen, habitsOpen, menuOpen, taskOpen]);
  ...
  return (
    <div className="hey-app">
      ...
      {route.view === "day" && <DayView state={state} setState={setState} route={route} navigate={navigate} onOpenEvent={openEvent} onNewAt={openNewAt} onEmail={() => setState((current) => beginEmailEventDraft(current, route.date))} onJournal={() => setJournalOpen(true)} onHabits={() => setHabitsOpen(true)} onAddTask={() => setTaskOpen(true)} onDecorate={() => setMenuOpen(true)} />}
      {route.view === "week" && <WeekView state={state} setState={setState} route={route} navigate={navigate} onOpenEvent={openEvent} onAddTask={() => setTaskOpen(true)} />}
      ...
      {searchOpen && <SearchDialog state={state} query={searchQuery} setQuery={setSearchQuery} onClose={() => { setSearchOpen(false); setSearchQuery(""); }} onOpenEvent={(eventId) => { setSearchOpen(false); setSearchQuery(""); openEvent(eventId); }} onGoDate={(date) => { setSearchOpen(false); setSearchQuery(""); navigate({ view: "day", date }); }} onGoSometime={() => { setSearchOpen(false); setSearchQuery(""); navigate({ view: "week", date: route.date }); announce("Sometime this week is shown between the current and next week"); }} />}
      ...
      {taskOpen && <AddTaskDialog state={state} setState={setState} onClose={() => setTaskOpen(false)} announce={announce} />}
