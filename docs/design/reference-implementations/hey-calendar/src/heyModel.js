...
const seedTasks = [
  { id: "task-gift", title: "Buy Mira's birthday gift", completed: false },
  { id: "task-flowers", title: "Send congratulations flowers", completed: true },
  { id: "task-flight", title: "Check for flight upgrades", completed: false },
];
...
export function toggleTask(state, taskId) {
  return { ...state, tasks: state.tasks.map((task) => task.id === taskId ? { ...task, completed: !task.completed } : task) };
}

export function addTask(state, title) {
  const clean = title.trim();
  if (!clean) return state;
  return { ...state, tasks: [...state.tasks, { id: `task-created-${state.nextTaskSequence}`, title: clean, completed: false }], nextTaskSequence: state.nextTaskSequence + 1 };
}
...
export function searchCalendar(state, query) {
  ...
  const taskResults = state.tasks
    ...
    .map((task) => ({ id: task.id, type: "Sometime", title: task.title, object: task }));
  ...
  return [...eventResults, ...taskResults, ...journalResults];
}
