...
function applyReferenceContext() {
  ...
  document.documentElement.dataset.reference = "agent-feed";
  document.documentElement.dataset.embedded = query.get("embedded") === "1" ? "true" : "false";
}

applyReferenceContext();

// Theme/scene changes arrive through the stable iframe. Updating URL and data
// attributes in place keeps the Agent Feed's local supervision state intact.
window.addEventListener("message", (event) => {
  ...
  for (const key of ["composition", "embedded"]) {
