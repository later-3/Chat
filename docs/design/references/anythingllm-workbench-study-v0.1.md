...
>
> 截图命名空间定义：
> - **ACW-** = `evidence/anythingllm-agent-constitution-workflow-v0.1/screenshots/`（25 张：ACW-01 Memory 设置入口 – ACW-25 Chat 工具菜单；覆盖 Memory / Skills / Flow / MCP / Scheduled Jobs / Survey / Agent Configuration）
> - **WB-** = `evidence/anythingllm-v0.1/screenshots/`（8 张：WB-00 总览 – WB-07 Open Computer Deliverable；覆盖 Conversation 起点 / 文件上下文 / Agent 进度 / 结果来源 / Open Computer）
>
...
| 页面中心 | 两种真正不同的模式：AnythingLLM 核心 = **Conversation-owned**；Open Computer = **Workspace-owned** | F · evidence card §2；O · WB-02–WB-05, WB-06–WB-07 |
...
| 最强可迁移机制 | Agent Configuration 独立工作面、Memory scope 可见、Skills/MCP/Flow 同页管理、Survey 对话内澄清、Scheduled Job 的 Trigger → Run → History 连续路径、Continue in Thread 返回路径 | O · 多张截图 |
...
**可见事实**（O）：Workspace 设置中有独立的 Agent Configuration 页面，可选择 Agent LLM provider 和 model，并链接到 Agent Skills。Agent 的模型与能力配置是**独立工作面**，不只是聊天输入框旁的设置图标；该画面不证明身份、角色、Owner 或 Participant。
...
**可见事实**（O）：单个 Run 同屏包含 Prompt、Thinking steps、Tool Calls、Files、Response、Metrics 和 **Continue in Thread**。Run detail 是 Agent Workflow Run 的独立、分节产品表面；Continue in Thread 明确提供 Run → Conversation 的返回路径。该画面不证明完整 Workflow 生命周期。
...
**可见事实**（O）：Run History 列出 status、started、duration、error。Scheduled Jobs 形成 Trigger → Run → History 的可见连续路径，但不等于覆盖 Plan、HITL、Pause/Resume、Reconcile 和正式写回等完整生命周期。
...
- 桌面/浏览器/文件现场居中，对话退到 sidecar（O · WB-06–WB-07）。
...
| 层 | AnythingLLM 事实 | 证据 |
|---|---|---|
| 作用域/导航 | 左侧 Workspace/Thread 列表；Settings 进入 Agent Configuration / Skills / MCP / Flows / Scheduled Jobs | O · ACW-23, ACW-10, ACW-05, ACW-18 |
| 主工作表面 | Conversation-owned（消息流）；Open Computer = Workspace-owned（桌面现场） | F · evidence card §2；O · WB-02, WB-06 |
| 上下文副表面 | Memory sidebar（Workspace/Global）；Sources 抽屉；Run detail 的 Thinking/Tool Calls/Files 区 | O · ACW-02, ACW-05, ACW-14 |
| 连续性 | Thread 保存对话上下文；Continue in Thread 提供 Run → Conversation 返回路径 | O · ACW-14 |
| 人工检查点 | Survey 对话内澄清；Run detail Stop；Chat composer 工具选择 | O · ACW-21, ACW-12, ACW-25 |
| 结果展示/返回路径（非正式 Evidence 写回） | answer + Sources；Run Files + Response；Continue in Thread 回到对话 | O · WB-05, ACW-14, ACW-16, ACW-17；U · 正式 Evidence/产品写回 |
| **Agent Configuration（扩展层）** | Workspace Agent 模型选择 + Skills 链接；全局 Skills/MCP/Flows 同页管理 | O · ACW-23, ACW-10 |
| **Workflow Definition（扩展层）** | Flow Builder 定义可复用 Flow；Scheduled Job 定义触发 + tools | O · ACW-05, ACW-18 |
| **Workflow Run（扩展层）** | Run detail 同屏 Prompt/Thinking/Tool Calls/Files/Response/Metrics | O · ACW-14 |
...
|---|---|---|
| **Identity / Role / Owner / Participant** | 不覆盖：Workspace 可选 Agent 模型属于能力配置，不证明 Agent 身份、角色、Owner 或 Participant；完整耐久 Agent Profile 也未出现 | O · ACW-23（模型配置）；U · Identity/Profile |
| **Capability / Skill / Tool / MCP / Model** | 覆盖：Agent Skills（含 Intelligent Selection）、Custom Skills、Agent Flows、MCP Servers 同页管理；Chat composer 可选工具 | O · ACW-10, ACW-04, ACW-25 |
| **Memory / Context / Source / Provenance** | 部分覆盖：Memory sidebar 有 Workspace/Global scope，记忆卡可见可管理；Sources 抽屉可引用；自动提取和模型注入细节未证明 | O · ACW-02（scope + UI）；D · 自动提取；U · 模型注入范围 |
...
|---|---|---|
| Goal input / Clarify / assumptions / scope | 部分覆盖：Survey 提供对话内结构化澄清 | O · ACW-21 |
| Editable Plan / confirmation | 不覆盖 | U |
| Flow definition / nodes / blocks / variables / tools | 覆盖：Flow Builder 提供完整定义表面 | O · ACW-05, ACW-06, ACW-07, ACW-08 |
| Configuration / version / publish | 部分覆盖：Configuration 可见（Agent 模型选择）；version/publish 未证明 | O · ACW-23；U · version/publish |
| Trigger: manual / event / schedule / object change | 部分覆盖：Scheduled Jobs 提供 schedule trigger；manual 通过 Run now；event/object change 未证明 | O · ACW-18, ACW-19；U · event/object change |
| Run / task / subtask / tool progress | 覆盖：Run detail 同屏 Thinking/Tool Calls/Files/Response/Metrics | O · ACW-14, ACW-15 |
| Checkpoint / ask-user / HITL / edit / accept / reject | 部分覆盖：Survey 是 ask-user；Stop 是控制入口；无 edit/accept/reject 中间 Artifact | O · ACW-21, ACW-12；U · Artifact accept/reject |
| Pause / resume / cancel / abort | 部分覆盖：Stop 提供 cancel/abort；Pause/resume 未证明 | O · ACW-12（Stop）；U · Pause/resume |
| Failure / timeout / retry / outcome_unknown / reconcile | 部分覆盖：Run History 显示 error status；具体 failure/retry 语义未证明 | O · ACW-11（error 列）；U · 完整 failure 语义 |
| Artifact / file / diff / Evidence review | 部分覆盖：Run Files 区展示生成文件；无评论/修订/接受拒绝 | O · ACW-16；U · review 闭环 |
| Delivery / writeback / notification | 部分覆盖：Response 区展示最终回复；Continue in Thread 提供返回路径 | O · ACW-17, ACW-14 |
| Run history / Continue in Thread / reuse / next schedule | 覆盖：Run History 列出历史；Continue in Thread 明确返回对话 | O · ACW-11, ACW-14 |

## 7. Chat 的 Take / Adapt / Refuse

### Take

1. **Agent 的模型与能力 Configuration 是独立工作面**，不只是聊天输入框旁的设置图标（O · ACW-23）；它不等于 Agent Identity/Profile。
2. **Memory 的 Workspace/Global scope 分离与可编辑 UI**（O · ACW-02）。
3. **Agent Skills/Flows/MCP 同页管理**减少配置跳转（O · ACW-10）。
4. **Survey 是对话内结构化 Checkpoint**，不跳设置页面（O · ACW-21）。
5. **Flow Builder 是独立 Definition 表面**，与 Conversation 和 Run 分离（O · ACW-05）。
6. **Scheduled Jobs 形成 Trigger → Run → History 的可见连续路径**（O · ACW-18, ACW-11, ACW-14），但不等于完整 Workflow 生命周期。
7. **Continue in Thread 是 Run → Conversation 返回路径**（O · ACW-14）。
...
2. **不把 Stop 写成 Pause/Resume**（O · ACW-12, ACW-13）。Stop 只证明终止入口。
...
| 场景 | 判定 | 证据 |
|---|---|---|
| Conversation-owned 对话型 Agent 工作区 | **覆盖** | F · evidence card §3；O · WB-02–WB-05 |
| Workspace-owned 电脑式执行工作面 | **部分覆盖**（保留 v0.1 结论） | F · evidence card §4；O · WB-06–WB-07 |
| Agent Configuration（模型选择 + Skills 链接） | **覆盖** | O · ACW-23, ACW-24 |
| Memory（Workspace/Global scope + 可编辑 UI） | **覆盖** | O · ACW-02 |
| Agent Skills / MCP / Flows 同页管理 | **覆盖** | O · ACW-10, ACW-04 |
| Agent Survey（对话内结构化澄清） | **覆盖** | O · ACW-21 |
| Flow Builder（Workflow Definition） | **覆盖** | O · ACW-05 |
| Scheduled Jobs（Trigger → Run → History 可见连续路径） | **覆盖** | O · ACW-18, ACW-19, ACW-11, ACW-14 |
| Continue in Thread（Run → Conversation） | **覆盖** | O · ACW-14 |
...
| 真正 Pause / Resume（Stop ≠ Pause/Resume） | O · ACW-12（Stop only）；U · Pause/resume |
...
| Flow version / publish / approval | U |
| Pause / Resume（Stop 只证明终止） | O（ACW-12 Stop）/ U（Pause/Resume） |
