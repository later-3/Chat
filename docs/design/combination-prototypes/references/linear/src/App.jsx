...
function basecampOwnsWork() {
  return compositionMode() === "room-basecamp";
}

function isLinearIssueRoute(route) {
  return route.view === "issues" || route.view === "issue";
}

function isProjectIssuesRoute(route) {
  return route.view === "project" && route.tab === "issues";
}
...
function postHostWork(route) {
  postHostRoute("work", { projectId: route.projectId, workId: route.issueId });
}
...
function Sidebar({ route, navigate }) {
  ...
  ].filter(([id]) => !(basecampOwnsWork() && id === "issues"));
  return (
...
export function App() {
  ...
  const navigate = (next, mode = "push") => {
    ...
    if ((basecampOwnsWork() && isLinearIssueRoute(complete)) || (combinationModes.has(compositionMode()) && isProjectIssuesRoute(complete))) {
      postHostWork(complete);
      return;
    }
  ...
  const delegatedWorkRoute = (basecampOwnsWork() && isLinearIssueRoute(route)) || (combinationModes.has(compositionMode()) && isProjectIssuesRoute(route));
  useEffect(() => {
    if (!delegatedWorkRoute) {
      delegatedRouteRef.current = "";
      return;
    }
    const key = `${route.view}:${route.projectId || ""}:${route.issueId || ""}:${route.tab || ""}`;
    if (delegatedRouteRef.current === key) return;
    delegatedRouteRef.current = key;
    postHostWork(route);
  }, [delegatedWorkRoute, route.issueId, route.projectId, route.tab, route.view]);

  let screen;
  if (delegatedWorkRoute) screen = null;
  else if (route.view === "issue") screen = <IssueDetail state={state} setState={setState} route={route} navigate={navigate} goBack={goBack} announce={announce} />;
