...
import {
  calendarById,
  createInitialState as createHeyState,
  eventsForDate,
  todayIso,
} from "../../hey/src/heyModel.js";

const heyTodayProjection = createHeyState();
...
function CalendarEvents() {
  const events = eventsForDate(heyTodayProjection, todayIso).filter((event) => projectedEventIds.has(event.id));
  const openCalendarEvent = (event) => {
    window.parent.postMessage(
      {
        type: "chat:navigate",
        scene: "calendar",
        context: { eventId: event.id, date: event.date },
      },
  ...
  return (
    <section className="calendar-events" aria-label="Calendar events">
      {events.map((event) => {
        const calendar = calendarById(heyTodayProjection, event.calendarId);
        return (
          <button
            type="button"
            className="event"
          ...
          >
            <span style={{ color: calendar.color }}>{event.start}</span>
