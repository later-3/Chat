...
依照[模块合同](../../architecture/chat-module-contracts.md)和[系统生命周期](../../architecture/chat-system-lifecycle.md)：服务定义属于原生管理器，配置/Session/Memory归属不变，VS Code任务与CLI消费同一脚本。范围必须显式选择；正常服务不能只kill PID。当前只实现明确中断的进程停止，未实现跨组件在途业务排空，不将后者标为完成。
