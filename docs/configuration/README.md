...
Workflow 终端的服务地址和启动参数见[命令行指南](../modules/tui/README.md)。它消费同一 Backend 配置，不新增模型、Workflow 或 Agent 配置源。

本文说明 Chat 的部署、模型、Workflow、Agent 和项目资源配置放在哪里，以及最常用的写法。它面向 Chat 用户和协助用户管理 Chat 的外部 AI；开发者若要修改配置机制，应另读[架构文档](../architecture/README.md)。

本文的 JSON、文件位置和 HTTP 接口描述当前实现。Long Agent 的完整目标见[使用与配置](../modules/long-agents/README.md)、[定义与配置模型](../modules/long-agents/chat-long-agent-capability-model.md)；已落地的独立配置/home 与日常会话轮换见下文；完整目标和剩余迁移以实施状态为准，不能直接向 schemaVersion 1 添加未实现字段。

共享项目概览、长期职责（灵魂模式）、自由活动和 Social 订阅的配置领域见[共享认知与自主工作](../modules/long-agents/chat-long-agent-awareness-and-autonomy.md)。本轮仅形成设计要求与机制评审稿，现有 JSON/API 尚不支持直接添加这些目标字段。

## 1. 配置范围

Chat 的配置入口按作用域分为进程/部署、Personal、Project、Session/本轮和产品源码。Chat 已删除产品登录；访问范围由部署环境管理，日常能力通常通过 Personal 与 Project 配置。Session 由 Chat 保存，产品源码配置只供开发内置 Workflow 和 Agent 时修改。
...
启用是显式创建动作，不新增全局开关：读取列表或重启不会强制生成助手，不会恢复已归档 Friend。重复启用返回已有 Friend。NanoClaw Host 必须已按[部署指南](../operations/README.md)以 `chat-pi` 启动并与 Backend 使用相同服务认证。无 Registry 时首次连接默认使用 `http://127.0.0.1:3000/webhook/chat-backend`、实例 `local`；自定义端口可通过 Backend 的 `CHAT_NANOCLAW_GATEWAY_URL` 指定。已有 Registry 继续使用其实例配置，不被环境变量覆盖。
...
Chat使用`<CHAT_HOME>/long-agents.json`登记长期Agent及其NanoClaw本机实例。该文件已降级为**索引**（身份、NanoClaw绑定、状态与 Channel 绑定）；每个 Agent 的能力定义位于独立配置根`<CHAT_HOME>/long-agents/<longAgentId>/definition.json`，资源目录（`skills/`、`prompts/`、`extensions/`）位于同一根下。存量内联 definition 会在首次读取时幂等迁移（备份与标记位于`runtime/migrations/long-agent-definition-split/`）。每个 Agent 只有一个根：`long-agents/<longAgentId>/{workspace,sessions,memory,skills,...}`，其 home 项目的 projectId 就是 `longAgentId`，日常主 Session 按持久化 IANA 时区的日期轮换。公共共享空间为 `longagentshare`（原共享 `daily` 归一而来，不默认打开）；普通会话必须属于用户自己的项目。`defaultProjectId`仅决定新建私聊绑定的默认协作上下文；Home/共享容器表示无业务项目。已有渠道使用自己的 `contextProjectId`；Web 每条消息显式选择项目或 null。所有 Friend 直接交流仍存入自己的每日 Session。Agent Memory 仍由 NanoClaw Agent Group 承载，迁入 Chat Home 是后续切片（S5b）。剩余差距见[实施状态](../modules/long-agents/chat-long-agent-roadmap.md)。

下面为导入兼容示例；读取后内联 `definition` 拆分到独立文件，日常管理使用 Web/API。
...
Web 管理接口为`GET/PATCH /api/long-agents/:id/agent-group`与`GET/PATCH /api/long-agents/:id/agent-memory`。Memory GET使用`operation=list|read|search`；PATCH使用`operation=write|delete`并要求`expectedRevision`完成乐观并发控制。搜索词不超过512个字符和32个空白分词；Markdown内容上限为900 KiB UTF-8数据，为Gateway JSON Envelope保留空间。接口只接受Memory根内的Markdown相对路径，不返回Gateway URL、Token或NanoClaw宿主绝对路径。Agent Group更新、Web Memory写入/删除和Pi `agent_memory_write`都会进入`<CHAT_HOME>/logs/audit.jsonl`，记录可信的Long Agent、Agent Group、Project/Session/Turn来源和结果Revision，但不记录Token或宿主路径。
...
Chat Web 的全局导航包括“同事、项目、动态、设置”。Friend 列表跨项目稳定存在；顶栏 Project 是当前交流的上下文，切 Project 不替换同事，切同事不改 Project。“项目”入口按当前 Project 展示普通 Session 和新建入口；普通会话归属于自己的 Project。Friend 模式保留原生 Long Agent Session 归属，顶栏上下文作为既有 `contextProjectId` 传入，不等于把 Session 移到另一 Project。首次无恢复目标时优先打开可用的 Nexus；明确的 Project 链接进入项目会话模式。右侧资料跟随当前 Project，历史文件预览保留来源。

`<CHAT_HOME>/long-agents/<longAgentId>/` 下按领域分开的受管可写文件：`definition.json`（Pi 运行策略）、`tasks.json`（LA2 任务定义、不可变修订与发生记录）、`duties.json`（LA3 职责、目标代号、进度与回执）、`artifacts.json`（LA4 产物记录、冻结内容与内容版本）。任务可选带 `deliverable`（`kind`/`slot`/`audience`）声明它是 3+1 产物任务，配置入口是任务表单与 `task_manage`；产物身份、提交、校验、恢复与状态合同见 [Friend 产物闭环](../modules/long-agents/deliverables.md)。这些文件只由 Backend 服务在文件锁内原子替换写入，不接受手工编辑，也不出现在 Frontend 响应中的宿主路径。

Long Agent配置入口位于“Friend”列表的设置按钮，优先选中当前 Friend。它编辑的`LongAgent`定义是Personal配置，对该Long Agent所有Project生效；顶栏上下文不会把同事定义变成Project配置。可编辑字段包括显示名称与描述、Personal全局启停、`defaultProjectId`、Model、Thinking Level、System Prompt/自定义Prompt、Tools和Resources；稳定`id`不可改名。
...
Chat Web 直接进入工作区，CLI/TUI 直接连接 Backend；产品登录页面、账号配置、Cookie 和登录接口已删除。普通产品 API 不再由 Chat 自带登录限制访问。公开部署的访问范围由部署环境管理，见[部署指南](../operations/README.md)。

模型 Provider 的 API Key/OAuth 和 NanoClaw 的服务 Token 保留，分别用于模型调用和服务间通信，不是 Chat 产品登录。
...
FastEmbed 不需要 API Key，模型缓存位于 `<CHAT_HOME>/cache/fastembed`。修改 Provider、模型或向量维度后，在 Memory 管理页对相关 Target 执行“重建索引”。Memory 的存储路径、读写流程和管理方式见[Chat 长期记忆](../modules/memory/README.md)。
...
Long Agent 未提供自定义 Definition 时，默认拥有 `project_search`、`project_read`、`project_create`、`project_open`、`project_update`、`project_configure`。已经保存显式 Definition 的 Agent 保留原选择；需要时在Friend配置页选择这六个 `system:tool/project_*` 地址，资源用 inherit 或明确选择 Skill 路径。普通 Workflow Agent 也可通过同一 Tool 配置入口启用。

例如直接说“创建思考项目，记录日常思考与反思”。新项目显示名为“思考”，工作目录为 `<CHAT_HOME>/workspaces/思考`，不添加 `project-` 前缀；同名目录已占用时使用 `思考 (2)` 等序号，显示名仍保留原名。名称支持中文和空格，须是有效的单级目录名，UTF-8最多240字节，不接受路径分隔符或控制字符。内部稳定projectId自动分配，也可通过可选id指定；Session、Memory等私有数据仍位于 `<CHAT_HOME>/projects/<projectId>`，不是用户工作目录。旧项目及同requestId重试保留原身份和目录，不自动迁移。`.chat/project.json` 保存身份和用途，`.chat/config.json` 默认继承。首次接入外部目录仍需先通过 Chat 项目选择器授权打开。

`project_read` 的 configuration 视图提供版本；`project_configure` 修改指定字段或通过 unset 恢复继承。创建结果提供进入新项目会话的链接，不修改当前会话归属。现有 Long Agent 的模型配置仍属于 Personal，不受 Project Workflow Agent 配置影响。完整参数和并发合同见 [Project 管理 Skill 与 Tool](../modules/projects/chat-project-management-design.md)。
...
新增或修改内置 Workflow 时，应遵守[Workflow 开发框架](../modules/workflows/chat-workflow-framework.md)并更新测试；不要为了改变一个项目的模型或资源而修改内置源码配置。
...
| 管理Long Agent身份、全局启停、默认Project与Agent能力 | 全局“Friend”入口的列表设置按钮；Personal事实在`<CHAT_HOME>/long-agents.json` |
| 修改Long Agent显示头像 | Friend 设置页的身份区域；图片资产在`<CHAT_HOME>/long-agents-assets/` |
...
该合同沿用Chat配置体系，避免脚本、VSCode、Nano服务和Web各自维护一份组件清单或启停策略。运行实例不是Long Agent；同一实例仍可承载多个独立Agent。普通 dev:all 管理 Backend/Vite；隔离 debug 可组合 Nano/TUI。Linux/WSL 的 chatctl 分开安装、组件启停与开机自启，可选管理 Nano 系统服务；完整跨组件业务排空尚未实现。安装路径与命令见[运维手册](../operations/running.md)。
...
Friend 每日唯一 Session、每轮项目冻结、schema 4 日历与队列、公共实时展示已实现。P5 无损迁移保留旧文件与索引备份，定义冲突会阻止覆盖，state 升级中断可补完成标记。旧 URL 仅通过精确迁移记录读取；不会全局搜索另一个项目作为失败回退。迁移目录、冲突处理和回退前提见[Friend 升级手册](../operations/friend-migration.md)。

Friend 有效模型从自身定义 → Personal 模型默认解析，不让浏览项目 `.pi/settings.json` 或旧 Session 模型隐式覆盖身份能力；普通 Workflow 保留原继承链。身份的 Chat/Nano 当前事实源保持不变，本轮不顺带迁移 Agent Memory 或双写受管字段。详细输入和迁移边界见[公共装配 §15](../architecture/chat-context-resource-model.md#15-公共-agent-装配合同p12026-09-19)、[Session §10](../modules/sessions/chat-session-architecture.md#10-friend-每日-session-与本轮项目p1待实施)。

P2 消息 API 的 `contextProjectId` 接受用户 Project ID 或 null，省略等同无项目；切换只作用于下一条消息。装配快照恢复使用原目标与规则，失效项目不回退 Home；无项目时工具 cwd 为专属 Workspace，Project Memory 默认写入要求明确目标。跨项目 `workflow_call` 保留父 Home Session 来源，子 Workflow 属于协作 Project。详见公共装配 §15.6。

Friend 配置 GET/PUT 的可选 `timeZone` 为 IANA 时区（例如 `Asia/Shanghai`、`America/New_York`），使用同一 revision 并发校验；旧配置首次使用固定 Backend 有效时区。配置页“运行策略”可修改，新请求采用新时区，已接受请求与历史不移动。日历与队列迁移、恢复路径见 [Long Agent 实现合同](../modules/long-agents/chat-long-agent-architecture.md#421-p3-实现合同2026-09-19)，不要手工改 Session 日期或 state 中的绑定来模拟换日。
