...
`<CHAT_HOME>/runtime/long-agent-state.json` schema 4 保存日历、耐久请求、当前投影、渠道绑定、待投递事件与幂等收据。202 前冻结并持久化接受输入，执行/Delivery/Ack 由恢复 Worker 负责；Session 读取不触发渠道重放。Web 用 `POST /api/long-agents/:id/turns` 接受文本/模型支持的图片，再按执行引用订阅；旧 messages API 仅同步兼容。失败重试与不明中断不能混淆，完整合同见[Friend 生命周期](../modules/long-agents/chat-long-agent-architecture.md#421-p3-实现合同2026-09-19)。
