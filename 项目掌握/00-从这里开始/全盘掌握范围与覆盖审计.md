...
> **当前准确状态**：覆盖骨架已经建立，不等于27课已经写完。现在有完整目录和缺口，但不少单元仍标记为
> “待补L1/L2”。后续Agent的任务是逐个消灭这些显式缺口，不能只在已有专题上继续加细节。
...
这11项来自待审核的目标总体架构。它们说明完整Chat最终必须负责什么；“有目标定义”不代表当前已经实现。
...
| M01 | 配置、组合根、应用工厂和生命周期 | 待补L1/L2 |
| M02 | React App Shell、Home、移动端、PWA和页面状态 | 待补L1/L2 |
| M03 | REST、DTO、RequestContext、错误和安全边界 | 待补L1/L2 |
| M04 | AG-UI请求、SSE、事件重放和端到端外层链 | 已有L2入口 |
| M05 | Product Session、Interaction、Message和历史恢复 | 已有L1 |
| M06 | Harness的Project、Work、Plan、Action、Note和Memory | 待补L1/L2 |
| M07 | TurnDigest、ContextPackage、来源采用和回合沉淀 | 已有L2入口 |
| M08 | Intent Set、Clarification、协作协议和StepInput | 待补L1/L2 |
| M09 | ExecutionDraft、RunSpec、HITL Policy、Decision、Grant和Approval | 待补L1/L2 |
| M10 | Agent Profile、Provider、ModelCallDraft、Attempt和精确请求 | 待补L1/L2 |
| M11 | Interaction协调和持续协作主Workflow的39节点闭环 | 已有L1，待逐路径L2 |
| M12 | MAF适配、Workflow目录、Checkpoint和5个辅助Workflow | 已有L1，待L2 |
| M13 | Product Run/Attempt、Runtime Job、Worker、Lease、Cursor和恢复 | 待补L1/L2 |
| M14 | Repository Binding、Snapshot、Context新鲜度和Git事实边界 | 待补L1/L2 |
| M15 | pi子进程、JSONL RPC、Provider Gateway、只读Tool和pi Session | 已有L1，部分L2 |
| M16 | Execution Workspace、ToolExecution、Operation、精确edit和对账 | 待补L1/L2 |
| M17 | Artifact、Evidence、Validation、Claim、Result Commit和Provenance | 待补L1/L2 |
| M18 | Product Finalization、双Trace、日志、Metrics和Diagnostics | 已有L1，待L2 |
| M19 | SQLite/Alembic、事务、CAS、幂等、Outbox和物理Store | 待补L1/L2 |
| M20 | 测试金字塔、质量门、故障实验、CI、依赖和供应链 | 待补L1/L2/L3 |
| M21 | 本地/分布式进程、VS Code调试、手机Relay和部署 | 部分L2 |
...
| M25 | 连续对话流、Conversation Day、Home、协作日历和Idea Garden | Home局部实现，其余待补 |
...
|---|---:|---|
| 后端Python源码文件 | 141 | 业务对象、适配器、Worker和入口都在这里，但文件数不等于能力数 |
| 后端顶层源码面 | 36 | 新增一个顶层能力目录/模块时必须归入M00-M26 |
| 前端源码文件 | 83 | 页面、Hook、投影和API Client都需要掌握 |
| 前端Feature目录 | 13 | 防止只讲后端、不讲用户实际看到和操作的界面 |
| 前端根源码面 | 35 | 防止散落在Feature目录外的入口、Store、Hook和Workbench漏掉 |
| 后端测试文件 | 48 | 合同、状态机、失败和恢复事实的证据来源 |
| 前端测试/E2E文件 | 24 | 投影、API合同、浏览器链和可访问性的证据来源 |
...
前端按13个Feature目录和35个根源码入口核对，覆盖Agents、Chat、Governance、Harness、Home、Mobile、
...
2. **已有样本级注释**：`CollaborationState`、TurnSummary、ContextPackage及建包服务附近已经说明状态所有权、
   内存/持久化边界和下游用途。
3. **尚未宣称完成**：M01-M26的其余入口没有完成逐单元“学习锚点”审计。代码原有Docstring可能符合工程维护
   标准，但不自动等于小白能沿链路学习。
...
| 9 | `intent_set_acceptance` | 接受或拒绝Intent候选 |
...
| 34 | `result_commit` | 提交经接受的结果事实 |
| 35 | `work_state_commit` | 更新Work状态 |
| 36 | `memory_commit` | 提交经接受的Memory |
| 37 | `harness_candidate_commit` | 提交Harness候选对象 |
| 38 | `turn_summary_persist` | 持久化TurnDigest/摘要 |
| 39 | `result_finalization` | 结束Product Run并生成终态投影 |
...
1. 27个学习单元ID唯一且引用文档存在。
2. 总体架构的11个目标模块全部有学习单元。
3. `backend/app`的全部36个顶层源码面都有归属。
4. `frontend/src/features`的13个目录和根目录35个源码文件都有归属。
5. Workflow目录的6个Workflow和主Workflow的39个节点逐项一致。
6. 10个运行/部署角色、9个协议边界和10个状态位置都有学习单元。
7. 测试、迁移、脚本、CI、部署和VS Code调试面存在且有归属。
...
| [`docs/overall-architecture-proposal.md`](../../docs/overall-architecture-proposal.md) | 待审核目标架构和11个目标模块 |
