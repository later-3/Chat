...
| M01 | 配置、组合根、应用工厂和生命周期 | 已有本地启动L1/L2入口；配置全集仍待补 |
...
| M08 | Intent Set、Clarification、协作协议和StepInput | 已有L2场景入口；专题仍待补 |
| M09 | ExecutionDraft、RunSpec、HITL Policy、Decision、Grant和Approval | 已有L2场景入口；专题仍待补 |
| M10 | Agent Profile、Provider、ModelCallDraft、Attempt和精确请求 | 已有L2场景入口；Agent专题仍待补 |
| M11 | Interaction协调和持续协作主Workflow的39节点闭环 | 已有完整S1–S7 L1和15场景L2入口 |
| M12 | MAF适配、Workflow目录、Checkpoint和5个辅助Workflow | 已有L1，待L2 |
| M13 | Product Run/Attempt、Runtime Job、Worker、Lease、Cursor和恢复 | 已有L2场景入口；专题仍待补 |
| M14 | Repository Binding、Snapshot、Context新鲜度和Git事实边界 | 已有L2场景入口；专题仍待补 |
| M15 | pi子进程、JSONL RPC、Provider Gateway、只读Tool和pi Session | 已有L1，部分L2 |
| M16 | Execution Workspace、ToolExecution、Operation、精确edit和对账 | 已有L2场景入口；通用对账仍待补 |
| M17 | Artifact、Evidence、Validation、Claim、Result Commit和Provenance | 已有L2场景入口；Provenance专题仍待补 |
...
| M21 | 本地/分布式进程、VS Code调试、手机Relay和部署 | 本地启动已有L1/L2；分布式与Relay为部分L2 |
...
每个场景单独成文并绑定pytest函数、输入族、预言机强度、关键对象/Store、必经/禁止节点和失败判定。
`scripts/check-project-mastery.py`会检查场景ID、文档元数据、测试函数和Workflow节点是否漂移。机器校验只能证明
映射存在；用户仍需用自己的Prompt完成手动断点与Trace对账，才能把个人掌握状态记为L2。
...
3. `backend/app`的全部37个顶层源码面都有归属。
4. `frontend/src/features`的13个目录和根目录38个源码文件都有归属。
5. Workflow目录的6个Workflow、S1–S7和主Workflow39个节点逐项一致。
