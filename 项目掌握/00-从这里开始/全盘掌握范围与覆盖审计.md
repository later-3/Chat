...
1. **目标责任账**：完整Chat最终要承担什么责任，共14个逻辑状态所有者、3个应用组件和3类运行时职责。
2. **当前实现账**：仓库今天真实存在什么，覆盖前端、后端、Workflow、进程、协议、存储和测试。
3. **学习单元账**：把前两本账拆成28个可验收单元，每个单元都要达到L1、L2或L3。
...
## 2. 目标责任账：14个状态所有者＋3个应用组件＋3类运行时职责

这20项责任来自已经批准的目标总体架构。它们说明完整Chat最终必须负责什么；“架构已批准”不代表每项
已经实现，也不能把字段、Schema、API和物理目录一并视为获批。

### 2.1 14个逻辑状态所有者

| # | 状态所有者 | 它要回答的用户问题 | 学习单元 | 当前边界 |
|---:|---|---|---|---|
| 1 | Identity与Channel Binding | 我是谁，从哪个入口来，有什么权限？ | M22 | 目标已定义，认证与外部Channel主链尚未实现 |
| 2 | Conversation | 会话、消息和Interaction怎样创建、查询和恢复？ | M05 | 已有Product Session主链，完整分支/跨入口仍缺 |
| 3 | Work | Project、Work、Plan和Action怎样持续推进？ | M06 | 已有Harness纵向链，独立公开合同待设计 |
| 4 | Knowledge | 项目资料、学习材料、Note与来源怎样累积？ | M06、M14 | 局部能力分散存在，独立合同未设计 |
| 5 | Protocol | 不同项目采用什么可版本化协作方法？ | M08 | 已有7类协议与Binding纵向链 |
| 6 | Collaboration Governance | Intent、候选、决定、授权和RunSpec怎样可改可审计？ | M08、M09、M10 | 核心纵向链已实现，边界仍待W4-01详细设计 |
| 7 | Context | 本轮采用了哪些信息、版本和预算？ | M07、M14 | 已有ContextPackage与仓库资源链 |
| 8 | Memory | 哪些候选可跨会话复用，怎样失效？ | M06、M07 | 已有候选/提交边界，完整失效传播未完成 |
| 9 | Run管理 | 失败、重试、重启、续接和接管各是什么意思？ | M13、M18 | 已有Job/Worker/Lease/Cursor主干 |
| 10 | Tool执行 | 外部读取与副作用怎样授权、幂等和对账？ | M15、M16 | pi与Tool/Operation主干已存在 |
| 11 | Evidence | 为什么“模型说完成”不等于Work完成？ | M17 | Artifact/Evidence/Validation/Claim链已存在 |
| 12 | Schedule | 每日/每周任务何时触发、漏跑怎么办？ | M06、M21、M25 | 所有权已批准，Schema/Worker未实现 |
| 13 | Delivery | 产品完成后怎样可靠送达Web或外部入口？ | M23 | 独立Delivery主链未实现 |
| 14 | Super Admin Operations | 怎样审计登录、使用、Work/Artifact进度和异常？ | M24 | 运营看护台未实现 |

### 2.2 3个应用组件

| 应用组件 | 回答的问题 | 学习单元 | 当前边界 |
|---|---|---|---|
| APP-INGRESS | 多入口请求怎样统一鉴权、去重并变成内部Envelope？ | M03、M04、M22、M23 | Web局部链存在，统一多Channel入口未实现 |
| APP-INTERACTION | 一次Interaction怎样跨所有者完成同一用例？ | M11 | 主Workflow有39节点；公开应用合同待W1-01 |
| APP-PROJECTION | Web、Obsidian和第三方前端怎样呈现同一权威事实？ | M02、M06、M23、M25、M27 | 固定Scope只读Envelope、Workspace、Dossier和Obsidian Adapter已实现；Identity裁剪/写回/大规模投影仍缺 |

### 2.3 3类运行时与基础设施职责

| 运行责任 | 回答的问题 | 学习单元 |
|---|---|---|
| RT-MAF | Agent、Workflow、Checkpoint和模型/Tool调用怎样运行？ | M10、M12 |
| RT-EXECUTION | Worker、Workspace、外部Runtime、Validator、Trigger和Reconciler怎样执行？ | M13、M15、M16、M17、M21 |
| RT-PLATFORM | Store、队列、Secret、部署、观测、备份、容量和灾难恢复怎样保障？ | M19、M20、M21、M26 |

## 3. 学习单元账：28个单元，一个都不能静默消失
...
| M27 | APP-PROJECTION、Personal Workspace、Project Dossier和Obsidian只读呈现 | L1专题＋L2合同/桌面/移动证据；Identity裁剪、双向写回和大规模投影仍缺 |

这张表回答“要学什么”；具体材料、源码归属和状态由机器清单维护，避免Markdown表与代码漂移。

## 4. 当前实现账：不能只盘点Python类

2026-07-30的当前工作树快照至少包含：

| 观察面 | 已纳入覆盖清单的数量 | 为什么单独盘点 |
|---|---:|---|
| 后端Python源码文件 | 148 | 业务对象、适配器、Worker和入口都在这里，但文件数不等于能力数 |
| 后端顶层源码面 | 38 | 新增一个顶层能力目录/模块时必须归入M00-M27 |
| 前端源码文件 | 96 | 页面、Hook、投影和API Client都需要掌握 |
| 前端Feature目录 | 16 | 防止只讲后端、不讲用户实际看到和操作的界面 |
| 前端根源码面 | 40 | 防止散落在Feature目录外的入口、Store、Hook和Workbench漏掉 |
| 后端测试文件 | 51 | 合同、状态机、失败和恢复事实的证据来源 |
| 前端测试/E2E文件 | 27 | 投影、API合同、浏览器链和可访问性的证据来源 |
...
| 当前/目标协议边界 | 10 | 每种合同拥有不同职责、失败语义和安全边界 |
| 当前/派生/外部/目标状态位置 | 11 | 防止Product事实、可重建Projection、Checkpoint、日志、浏览器和外部状态混用 |
...
Readonly Tools、pi Gateway/Runtime/Session、Project Resources、Observability、Home、Projection、E2E和各类Worker。
...
前端按16个Feature目录和40个根源码入口核对，覆盖Agents、Chat、Governance、Harness、Home、Mobile、
Model Call Review、Protocols、Projections、Projects、Workspace、Session、Settings、Shared、Tools、Workflow，以及App Shell、API Client、
...
4. **尚未宣称完成**：M01-M27的其余入口没有完成逐单元“学习锚点”审计。代码原有Docstring可能符合工程维护
   标准，但不自动等于小白能沿链路学习。
...
| Obsidian Projection文件协议 | `project-dossier.v1`转换为稳定ID的Markdown/Manifest/ZIP | 文件不是Project事实源，当前编辑不写回 |
...
Git仓库、pi Session JSONL、进程日志、浏览器草稿/页面状态、即时生成的Projection Envelope/ZIP和外部Provider状态。
其中Projection Envelope/ZIP是派生且可重建的，不是新的权威Store。掌握M19时，要能回答每类状态的
...
| 2. 看懂协作决策与同源呈现 | M06-M12、M27 | 能解释Context、Intent、Plan、Approval、ModelCall、全部Workflow和Web/Obsidian投影 |
...
1. 根README能到唯一课程入口，01–61课号连续，全部61篇学习者文档恰好排课1次。
2. 28个学习单元ID唯一且引用文档存在。
3. 总体架构的14个状态所有者、3个应用组件和3类运行时职责全部有学习单元。
4. `backend/app`的全部38个顶层源码面都有归属。
5. `frontend/src/features`的16个目录和根目录40个源码文件都有归属。
6. Workflow目录的6个Workflow、S1–S7和主Workflow39个节点逐项一致。
7. 10个运行/部署角色、10个协议边界和11个状态位置都有学习单元。
...
| 文件 | 职责 |
|---|---|
| [`项目掌握/coverage-manifest.json`](../coverage-manifest.json) | 28单元与目标模块、源码面、Workflow和质量面的机器映射 |
...
| [`docs/overall-architecture-proposal.md`](../../docs/overall-architecture-proposal.md) | 已批准的14+3+3目标责任架构 |
...
- 2026-07-30：APP-PROJECTION只读切片落地后新增M27与第17课；覆盖账更新为28学习单元、38个后端顶层面、
  16个前端Feature、40个前端根面、10个协议边界和11个状态位置，并明确Envelope/ZIP是可重建派生物。
