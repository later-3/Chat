...
这里的“五层”只是**同一份数据变形时的5类对象形态**，不是另一套系统架构层，也不替代七层依赖图。它按下面的责任变化得出：

```text
页面要怎样显示和操作（View Model）
→ 跨网络允许传什么（DTO / Event）
→ 服务端验证后准备做什么（Envelope / Command）
→ 产品长期承认什么（Domain Object）
→ 为执行与恢复需要什么临时控制状态（MAF / Worker Runtime Object）
```

每跨越一次，可信程度、所有者、生命周期或可变方式就发生了变化，因此不能用同一个万能`ChatState`类贯穿全链。
...
| `docs/overall-architecture-proposal.md` | 对象所有权、ID链和14个状态所有者＋3个应用组件＋3类运行时职责基线 |
...
| `backend/app/governance/`、`backend/app/collaboration_protocols/` | Draft、RunSpec、Decision和Grant的当前合同/治理子边界 |
| `backend/app/runtime_execution/` | Job、Lease、Cursor和恢复对象 |
