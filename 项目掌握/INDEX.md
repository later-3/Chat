...
| 课 | 文档 | 本课完成标准 |
|---:|---|---|
| 39 | [breakpoint()与Python调试器第一性原理](./调试实战/breakpoint与Python调试器第一性原理.md) | 能从C++视角解释Python breakpoint()底层、VS Code debugpy接管、协程断点和BP编号体系 |
| 40 | [从断点停住到知道来路和下一跳](./调试实战/从断点停住到知道来路和下一跳.md) | 能在任一断点回答调用者、当前数据和下一跳 |
| 41 | [一句Prompt如何逐层变成Chat对象](./调试实战/一句Prompt如何逐层变成Chat对象.md) | 能沿同一Run解释字段在哪一层被增加或转换 |
| 42 | [断点清单与调试配置](./调试实战/断点清单与调试配置.md) | 能按场景选择小断点组合，不在默认运行中卡住 |
| 43 | [场景实验室使用方法](./调试实战/00-场景实验室使用方法.md) | 能先写预言机，再用断点、Store和Trace验收 |
| 44 | [第1课：从点击发送到ContextPackage](./调试实战/第1课-从点击发送到ContextPackage.md) | 完成第一条可重复的S1数据实验 |
| 45 | [第2课：端到端全链调试与Trace使用](./调试实战/第2课-端到端全链调试与Trace使用.md) | 能复用公共断点和Trace工具定位分叉 |
| 46 | [第3课：真实运行全链透视](./调试实战/第3课-真实运行全链透视.md) | 会区分历史实测样本和当前可重复合同 |
| 47 | [15个场景教材深度审计与升级顺序](./调试实战/15个场景教材深度审计与升级顺序.md) | 先看清每个场景是L2实值、L1+受控证据还是缺口 |

### 第七段：用自己的输入逐场景验证（48–62）

| 课 | 场景 | 当前教材证据 |
|---:|---|---|
| 48 | [SC01：确定性查询正式Project目录](./调试实战/场景/SC01-确定性查询正式Project目录.md) | L2+：当前真实确定性Run与精确合同 |
| 49 | [SC02：普通问答与三次模型调用治理](./调试实战/场景/SC02-普通问答与三次模型调用治理.md) | L2：当前真实3次ModelCall Run |
| 50 | [SC03：歧义请求与跨回合澄清](./调试实战/场景/SC03-歧义请求与跨回合澄清.md) | 第一轮L2；跨回合部分L1+ |
| 51 | [SC04：多Intent与组合Plan](./调试实战/场景/SC04-多Intent与组合Plan.md) | L1+：受控集成Fixture |
| 52 | [SC05：修改Context与revision失效](./调试实战/场景/SC05-修改Context与revision失效.md) | L1+：受控集成Fixture |
| 53 | [SC06：Repository来源失效零发送](./调试实战/场景/SC06-Repository来源失效零发送.md) | 失败路径L2：当前真实失败Run与零新发送合同 |
| 54 | [SC07：编辑模型请求与重新审批](./调试实战/场景/SC07-编辑模型请求与重新审批.md) | L1+：字节精确受控Fixture |
| 55 | [SC08：放弃模型调用零发送](./调试实战/场景/SC08-放弃模型调用零发送.md) | L2：当前真实放弃Run与零发送合同 |
| 56 | [SC09：受治理pi只读检查](./调试实战/场景/SC09-受治理pi只读检查.md) | L2：当前真实pi进程与Tool Run |
| 57 | [SC10：隔离精确编辑与Evidence提交](./调试实战/场景/SC10-隔离精确编辑与Evidence提交.md) | L1+并保留真实缺口样本；不要误判成完整成功 |
| 58 | [SC11：Validation失败与结果未知](./调试实战/场景/SC11-Validation失败与结果未知.md) | L1+：受控故障注入 |
| 59 | [SC12：断线后Worker继续与Cursor接回](./调试实战/场景/SC12-断线后Worker继续与Cursor接回.md) | L1+：真实Runtime组件与受控断线 |
| 60 | [SC13：取消与结果未知](./调试实战/场景/SC13-取消与结果未知.md) | L1+：受控产品状态机 |
| 61 | [SC14：Retry与Restart血缘](./调试实战/场景/SC14-Retry与Restart血缘.md) | L1+：受控Product Run血缘 |
| 62 | [SC15：跨进程HITL恢复](./调试实战/场景/SC15-跨进程HITL恢复.md) | L1+：受控多进程SQLite与Worker |

### 第八段：12层代码深潜与参考项目研究（63–67）

| 课 | 文档 | 本课完成标准 |
|---:|---|---|
| 63 | [一次发送穿过哪些对象（12层架构总览）](./00-从这里开始/run_once_example.md) | 能用一次真实发送复述12层每层是什么、为什么需要、框架与自研怎么分 |
| 64 | [一次发送的代码流程深入：12层每一步真实代码](./00-从这里开始/一次发送的代码流程深入-12层每一步真实代码.md) | 能沿同一真实Run逐层指出真实代码、停住后看什么变量、数据怎样变化、接力棒ID怎样串联 |
| 65 | [TencentDB Agent Memory 架构与流程](./参考项目研究/TencentDB-Agent-Memory架构与流程.md) | 能用28条真实Session例子复述L0→L1→L2→L3→Skill→Proxy装配每层产物和失败分支 |
| 66 | [MemOS 架构与流程](./参考项目研究/MemOS架构与流程.md) | 能用`s-py-e2e`例子复述Episode/L1/L2/L3/Skill实跑值、正负证据门和14个模块源码落点 |
| 67 | [memmy-agent 架构与流程](./参考项目研究/memmy-agent架构与流程.md) | 能用草莓回合和4回合演化复述turn.start/complete、单一事务、Worker claim/lease/dead-letter和Trial闭环 |
...
| 连续课程文档 | 67篇（01–67） | 本页`project-mastery-route`区块 |
...
| [一次发送穿过哪些对象（12层架构总览）](./00-从这里开始/run_once_example.md) | 从前端到后端到发给大模型，12层每层是什么、为什么需要、前后端/框架/自研怎么分 |
| [一次发送的代码流程深入：12层每一步真实代码](./00-从这里开始/一次发送的代码流程深入-12层每一步真实代码.md) | 把12层每层打开：真实代码、停住看什么变量、数据怎么变化、接力棒ID怎么串联 |
...
| 文档 | 解决的问题 |
|---|---|
| [TencentDB Agent Memory 架构与流程](./参考项目研究/TencentDB-Agent-Memory架构与流程.md) | 从C++视角理解它怎样采集会话、分层记忆(L0-L3/Skill)、编译Loadout和召回；固定提交f3df793 |
| [MemOS 架构与流程](./参考项目研究/MemOS架构与流程.md) | 从C++视角理解MemOS Local v2的Episode->L1->L2/L3/Skill分层、Tier 1/2/3检索和异步演化；固定提交027dc89 |
| [memmy-agent 架构与流程](./参考项目研究/memmy-agent架构与流程.md) | 从C++视角理解memmy的Agent Hook->公共HTTP->SQLite事务->Worker->L2/L3/Skill链和claim/lease/retry；固定提交211d521 |
...
本页`project-mastery-route`区块维护01–67的第一次学习顺序，反向覆盖全部学习者Markdown；
