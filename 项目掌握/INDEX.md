...
1. [从C++到Chat：前后端怎样跑起来](./00-从这里开始/从C++到Chat前后端怎样跑起来.md)：先弄懂源码、运行时、进程、端口、HTTP/JSON/SSE，亲手启动并观察前后端。
2. [Chat系统总地图与学习方法](./00-从这里开始/Chat系统总地图与学习方法.md)：建立一次点击的全链心智模型。
3. [Chat总体架构与一次点击的七层链路](./架构与模块/Chat总体架构与一次点击的七层链路.md)：完成“产品问题→用户场景→风险/保证→七层边界→13个候选责任→11模块→技术选型→当前源码”的架构前置课。
4. [11个产品模块](./架构与模块/11个产品模块的职责与代码落点.md)：重算13个候选为何经2次合并形成11个模块，再理解各模块对象、独立边界、当前落点与优化信号。
5. [核心对象词典](./架构与模块/核心对象词典-谁创建谁保存谁消费.md)：认清View、DTO、产品事实和Runtime对象及其所有者。
