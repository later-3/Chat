...
B2已经通过PR #7合入，M1真实memmy查询通过PR #10合入；M2真实显式导入候选已完成，正在PR #11最终合入。下一阶段仍围绕三个用户结果推进：
...
当前唯一下一实现任务是用固定版本Tencent MemoryCore完成第二个真实查询/导入Adapter，证明M1/M2的Port能承载L0接收、L1异步物化、serviceId与强隔离差异；不得用fake服务、也不得把memmy私有字段扩成万能接口。完成第二后端后，再分别进入BMAD项目上下文和用户规则纵向任务。
